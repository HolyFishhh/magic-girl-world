...
## 2026-08-28：综合玩家更新与正式发布整理（0.5.148）

- 内部版本升级为 `0.5.148`，新增面向玩家的 `beta0.2.0` 综合更新记录，统一概括 `beta0.1.6` 之后的自由开局、两阶段生成、多轮成长、卡牌机制、战斗 UI、变量提示与长期稳定性改造，不公开源码和内部协议细节。
- 实验性的卡牌与敌人生成知识库继续留在 `TODO.md`，不进入角色卡；正式 manifest 保持 20 条世界书。
- `npm run release:tavern` 完整通过，用时约 `257.5s`。确定性长流程覆盖首轮初始化、首战胜利、奖励领取、继承成长后的第二次遇敌和多项战败惩罚。
- 通过 SillyTavern 官方接口导入并读回角色 `魔法少女世界 0.5.148`、世界书 `魔法少女世界0.5.148`、20 条世界书、7 条正则和 2 个已授权角色脚本。最终唯一交付卡为根目录 `魔法少女世界.png`，大小 `8,045,045` 字节，SHA-256 `531C0B3556EA018A7AE738DF925D8D74BB1E9F0E70735AA2F5E7652C8911A285`。

## 2026-08-27：剧情模式最终闭环、战斗 UI 实机收尾与 MVU 提速（0.5.130）

- 将胜负后的变量生成从剧情提示中彻底分离：剧情模型只收到已确定的玩家状态、敌方状态和完整战斗日志；奖励预算与失败惩罚只写入 `reward.request`，由带 `[MVU_BATTLE_SETTLEMENT]` 锚点的第二阶段条目处理。胜利固定生成足量候选并清理事务，战败不生成奖励且只登记一种持久惩罚。
- 使用本地 SillyTavern 1.18.0、酒馆助手 4.9.3、玩家导入的剧情预设和 `deepseek-v4-flash` 完成首轮初始化、普通增量、第一次遇敌、胜利领奖、第二次遇敌、战败惩罚六阶段连续测试。普通轮没有重复初始化卡组，两个敌人都通过战斗预检，奖励领取后继续成长，战败后仍能通过下一轮就绪检查。
- 第二阶段不再重复携带两份完整当前变量：`5变量更新.MD` 仍常驻提供全部快照，`1变量数据结构.md` 只保留结构和职责，不再复制动态快照。thinking 默认关闭、历史保留 2 层、最大回复 2600 token、顺序重试最多 2 次。实测角色世界书输入约为：普通更新 2377、首轮 8273、遇敌 8279、战斗结算 6651 token；速度瓶颈不再包含重复牌组快照和可见思考链。
- MVU 顶部状态窗在非流式请求中显示旋转加载，完成后把更新命令重组为“当前时间：旧值 → 新值”“卡牌：新增名称”等自然语言变化，不再向玩家展示原始 `UpdateVariable`。完成态不会被迟到的应用事件覆盖；设置悬浮球与状态窗保持独立并可拖动、限位和持久化。
- 战斗 UI 恢复 2.0 以前的深蓝星空背景，同时保留新的横向排版和非阻塞 Emoji 战斗舞台。攻击、技能、能力、诅咒和遗物使用不同舞台动画；伤害、治疗和欲望数字改在舞台浮动。卡牌详情绑定当前卡牌或拖动幽灵，Pointer Events 只移动一张卡，不再触发浏览器原生整组拖动。
- Buff、欲望效果、能力、遗物和敌人意图均可打开完整详情；修饰符重做为“我方 / 属性 / 敌方”三列实时对照。完整牌组、抽牌堆、弃牌堆和消耗堆入口恢复，牌堆详情与普通状态栏继续复用同一中文效果解析器。战斗日志保存整场，只限制可见 DOM 数量，交接时不再截断为末尾十几轮。
- 最终卡已通过 SillyTavern 官方接口导入并读回为显示名 `魔法少女世界 0.5.130`：内嵌世界书 `魔法少女世界0.5.130` 共 20 条、角色正则 7 条、角色脚本 2 个，正则与脚本授权均开启。为复核作者备注而在本地重复导入同一版本时，酒馆把文件保存为 `魔法少女世界 0.5.1301.png`，角色显示名和内嵌内容保持不变；用户从旧版本首次导入时不会出现这个本地测试后缀。真实酒馆中的战斗 iframe 在 519×640 视口下 `scrollWidth/clientWidth=519/519`、`scrollHeight/clientHeight=640/640`；开始页、常态状态栏和战斗页均完成实际 DOM 与点击验收。
- 完整 `npm run release:tavern` 用时 `421.6s` 并通过；随后仅修正旧作者备注并重新执行角色卡补丁与 Tavern 契约校验。最终角色卡为 `7,726,885` 字节，SHA-256 `A8978E974ACDC291C0074740B0E3F8B51373A82CB4086E1F5DD8DB6B28B37D0B`；角色运行时为 `912,069` 字节，SHA-256 `6981E0BB6B43DE28C61DCA9A31F1CE7549B2D20DFF6A36D776243EF6051D917D`。

## 2026-08-27：剧情模式完整闭环、运行时去重与战斗信息重排（0.5.128）

- 修复角色卡在长对话中越来越卡乃至重新进入后卡死的问题：父窗口只保留一个运行时实例，旧实例会注销酒馆事件、DOM 事件、轮询器、战斗协调器与伤害数字队列；伤害数字不再为每个数字创建持续三秒的独立动画循环。
- 修复 MVU 状态窗在完成后回退为“正在应用变量”：完成态不会再被迟到的应用事件覆盖；非流式请求期间保持旋转加载，完成后再显示完整变量原文。设置悬浮球支持拖动、视口约束和位置持久化，且与自动展开的 MVU 状态窗保持独立。
- 战斗页保留横向“敌方—舞台—我方—手牌”结构并提高可用高度；移除误导性的横向拖动框。卡牌名称与类型同级显示，并由“稀有度 + 类型”共同着色；诅咒牌不可主动打出。遗物、能力、欲望效果、状态和敌人意图均可点击查看完整中文效果，能力同时显示来源。
- 修饰符展示改为显式传入玩家或敌方目标，解决两侧都显示“受伤修饰符”的身份误判；战斗计算仍使用同一修饰符核心。效果标签统一使用“敌方”，重复命中合并显示，普通状态栏、战斗页和奖励页继续复用同一解析器。
- 剧情模型长期读取所有持久变量，仅排除模式锚点、远征状态、当前敌人、临时战斗态与奖励事务；MVU 第二阶段读取完整快照，但普通回合只允许增量写入，不再反复初始化卡组。
- 遇敌规范要求继承剧情中已经发生的攻击、欲望和状态变化：若剧情明确命中或擦伤，进入战斗时的当前生命必须相对注入旧值发生实际变化，不能再用“本来就非满血”冒充承伤。敌人行动和能力可通过 `creates + add_card` 向玩家本场牌区插入临时卡牌。
- 战败交接通常只登记一种带敌人主题的诅咒牌、永久负面状态或负面遗物；战斗结束交接固定包含生成要求、玩家全部属性与当前状态、敌方状态及具体日志，并清除普通状态栏的自定义行动输入框。
- 使用本地 SillyTavern 1.18.0、酒馆助手 4.9.3、玩家导入的 `夏瑾_天琴座_Beta_3.1 (qz改 推荐使用flash模型)` 预设和 `deepseek-v4-flash` 完成真实六阶段闭环：首轮初始化、普通增量、第一次遇敌、胜利奖励与领取、第二次遇敌、战败惩罚全部通过。第二次遇敌继承剧情先手攻击，玩家生命 `90→84`；战败没有奖励，并只新增一个负面遗物。
- 完整 `npm run release:tavern` 用时 `141.5s` 并通过。最终角色卡为 `7,698,029` 字节，SHA-256 `770EC2E71A39660BF8E3DF9D3504E5B29FADF3511400628CCFF04CB0B1031FCD`；角色运行时为 `895,493` 字节，SHA-256 `D0DDEE38F01CD915678A8C744A3F1D2B822A16C6D973568064FDA47FDB73AC2F`。
- 新卡已通过 SillyTavern 官方接口导入并激活为 `魔法少女世界 0.5.128.png`。酒馆接口读回确认：版本 `0.5.128`、内嵌世界书 `魔法少女世界0.5.128`（19 条）、7 条角色正则、`MVU变量框架` 与 `魔法少女世界运行时` 两个角色脚本，以及正则和角色脚本授权均存在。

## 2026-08-27：真实两阶段多轮闭环与 AI 输入收敛（0.5.127）

- 使用本地 SillyTavern 8012、玩家导入的 `夏瑾_天琴座_Beta_3.1 (qz改 推荐使用flash模型)` 预设和 `deepseek-v4-flash` 完成真实四阶段连续生成：首轮初始化、普通增量、自然遇敌、战后奖励与领取全部通过。
- 普通轮没有再次初始化卡组；首轮为 12 张牌、1 遗物、1 道具，遇敌预检无警告。程序结算 25 经验后生成 3 选 1 卡牌与 1 选 1 道具，领取后牌组增至 13、道具种类增至 2，敌人、奖励候选和限制均正确清空。
- AI 输入边界新增两项窄规范化：对象式 `apply_status/remove_status` 转为规范同级字段；单个 `scry/seek` 与常见数值效果同对象时按固定顺序拆分。歧义组合、冲突字段、未知状态和多个牌区操作继续报错。
- 首轮初始化改为多条短 `assign`，公式常量继续限制一位小数；新增深度 0 剧情交接终检，降低超长数组和战斗标记遗漏概率。
- 真实验收脚本支持分阶段留证与恢复，不会在后段失败时重复消耗已成功的模型请求；解析行为与卡内锁定的 MVU commit 对齐。
- 完整 `npm run release:tavern` 用时 `321.4s` 并通过。最终角色卡为 `7,696,045` 字节，SHA-256 `72CFB7AA39DE9DD3A0F9BF00975BD679208EFB961464FD69A05E95E62F6A8E41`；角色运行时为 `896,533` 字节，SHA-256 `8CAAF5C75ECA5695CE4FC039B920398E4CF9B17B6854F4C2079DF70CD96D27AC`。
- 新卡已通过官方接口导入本地酒馆为 `魔法少女世界 0.5.127.png` 并自动设为当前角色；从酒馆读回确认版本 `0.5.127`、内嵌世界书 `魔法少女世界0.5.127`（19 条）、7 条正则、MVU 与角色运行时两个脚本及角色授权均存在。

## 2026-08-27：战斗信息完整显示与目标动画（0.5.126）

- 战斗 iframe 固定高度由 600px 提升到 740px，并利用酒馆助手的 `body.scrollHeight` 自动同步楼层高度；敌我详情区增加可滚动余量，避免欲望效果、状态、能力和遗物被紧凑布局直接裁掉。
- 欲望效果改用与遗物、能力共用的独立详情浮层，可查看完整中文说明与解析后的效果标签。
- 战斗剧场移除动作 Emoji 的圆形边框。攻击动作仍跨场命中；技能、能力、事件、诅咒与遗物根据共享效果程序判断 `self/opponent`，在实际目标身上缓慢浮现，动画保持非阻塞，不降低连续出牌速度。
- 失败结算新增一次性 `[战败惩罚]` 交接。第二阶段通常只登记一种与敌人主题相关的诅咒牌、永久状态或负面遗物，且不混入胜利奖励。
- 已增加目标判定、战斗展示边界、战败提示与世界书契约回归测试。
- `0.5.126` 已通过完整发布链、便携包、角色运行时、Tavern 契约与真实 8012 导入验证；最终角色卡 SHA-256 为 `6A3883E5C962CBEAA3D81418B37F66177570C7B1682863EDFE670478EADC1446`。酒馆保存后的独立角色包含 18 条世界书、7 条正则和 2 个角色脚本。

## 2026-08-27：MVU 官方生命周期加载窗（0.5.125）

- 修复第二轮实际生成时顶部加载窗没有自动打开：不再改写酒馆助手的 `generateRaw`，改为监听 MVU 在额外模型请求前后写入的全局 `extra_analysis` 状态，因此内置破限、重试和模块内部请求都能触发同一加载窗。
- 接入 MVU 的 `COMMAND_PARSED` 事件，在模型完整返回并进入变量解析后提取最后一个 `UpdateVariable` 块；非流式请求期间保持旋转加载态，变量原文和可用的思考摘要只在完整返回后显示。
- 当额外模型请求结束但 MVU 没有继续解析或写入时，状态窗会在等待超时后明确显示失败，不再无限停留或悄悄消失。
- `0.5.125` 已通过类型检查、MVU 卡内加载器、开始流程、角色运行时自动生命周期、生产构建、Tavern 导出与最终契约校验，并通过 SillyTavern 官方接口导入本地 8012 实例为独立角色 `魔法少女世界 0.5.125.png`。
- 最终角色卡 `魔法少女世界.png` 为 `7,675,709` 字节，SHA-256 `35420D497CF382717562F1E35DD35AA669A4AD4D6C1430A2F8A3EE20BA9EA77B`；角色运行时为 `891,835` 字节，SHA-256 `7FF3802A34A5F608A931DBC34C004CAA0FDEACDB3A9E28BCE53FEC795D4C4E24`。

## 2026-08-27：设置悬浮球、独立 MVU 窗与普通页效果（0.5.124）

- 将此前混在同一面板里的设置与 MVU 状态彻底拆分：右上角常驻图标悬浮球只打开紧凑的通用设置页；MVU 二阶段拥有独立的顶部状态窗，仅在“自动显示变量生成窗”启用时随请求自动打开。
- 设置项改为完整横向行和自绘开关，不再依赖容易被酒馆全局样式挤压的原生复选框；悬浮球不再显示会折成两行的“设置”文字，生成期间会在球上显示忙碌动画。
- 非流式第二轮在生成和变量写入阶段只显示双环旋转指示、“正在生成变量/正在应用变量”和耗时；思考、更新摘要与变量原文只在完整结果完成或失败后展示，不把假流式片段冒充最终内容。
- 普通常态页的牌库、遗物和道具接入与战斗、奖励相同的共享效果解析器，显示中文可执行效果标签及可选描述；结构化 `trigger:{on,effects}` 的即时效果和持续触发效果会同时展示，页面不再只有名称、费用和叙事描述。
- 修复第二阶段在普通轮反复初始化牌组：常驻规则不再要求模型检查未注入的完整集合；首条初始化只扫描当前回复的 `<CHARACTER_INIT_PENDING>`，不再被历史 `[开始游戏]` 激活；初始化成功后卡内运行时立即消费该一次性标记。普通轮对持久战斗内容禁止整组覆盖，成长只通过明确内容标记和 `assign/remove` 增量发生。
- 剧情模型的长期 MVU 读取保持每轮常驻，包含服装、剧情物品、自定义状态定义和欲望效果；`enemy/reward/player_abilities/player_status_effects` 等一次性事务对象不占普通剧情上下文，`game_mode/run` 由程序锚点切换世界书，剧情模型也不读取。第二阶段 MVU 更新模型则每轮读取完整 `stat_data` 快照，包括一次性对象和模式状态，但只有相应标记出现时才获得复杂生成规范并允许增量写入。“完整读取”与“最小写入”分别设有契约门禁。
- `0.5.124` 已通过类型检查、世界书职责/上下文边界、MVU 卡内加载器、共享效果展示、普通状态栏、角色运行时、生产构建和最终 Tavern 契约校验，并通过 SillyTavern 官方接口导入本地 8012 实例为独立角色 `魔法少女世界 0.5.124.png`，角色正则与角色脚本已启用。
- 最终角色卡 `魔法少女世界.png` 为 `7,672,973` 字节，SHA-256 `3ACFF8D7BF148B0AF28B556626C7CF527F7D159FEA8865CF8FECE5CCEE4705D0`；角色运行时为 `889,663` 字节，SHA-256 `1620539DD1E1EBDC7CA4B342E2CA5FCE9EB34069E3F98A40DC31C93D912D62A9`。Codex 可视浏览器控制仍因桌面环境缺少可写浏览器内核资源路径而无法连接，所以本轮确认的是官方导入、保存权限和运行时/世界书契约，不把静态构建当作视觉点击验收。

## 2026-08-27：设置中心自动展开与结构化触发器（0.5.122）

- 顶部入口改为通用“设置”按钮；MVU 二阶段开始时无条件自动展开，非流式或假流式响应在生成期间只显示加载状态，完整返回后才展示思考/摘要与变量原文。
- 默认请求模型接口返回可展示的思考内容；接口不提供 reasoning 时给出明确说明，并可从非占位的 `<Analysis>` 中提取更新摘要，不伪造思考过程。
- 卡内加载器持续检查酒馆助手的 `generateRaw` 引用；MVU 或其他扩展重新挂载该函数后会自动恢复监控，修复设置窗只能手动打开的问题。
- Power、遗物与能力正式支持把时机和行为收在一起的结构化 `trigger:{on,effects}`。Power 可同时拥有打出时即时 `effects` 与持续触发效果，内容预检、运行时转换、奖励校验和构筑分析共用同一语义。
- 全局 UI 恢复旧版使用的 ZCOOL KuaiLe 手写字体链，数值回退字体保持清晰；新增结构已通过精确报错样本回归。
- 完整 `npm run release:tavern` 用时 `172.6s` 并通过。最终角色卡 `魔法少女世界.png` 为 `7,644,053` 字节，SHA-256 `E73F2AA7BF168DC1AB78B0C29BA45F1FD5381493EEB126FA37B8C443DF78B874`；角色运行时为 `882,129` 字节，SHA-256 `79B9164BCD739D43262269B198AF12BD6659D4B8CF38F589C5CECF61329E6EEF`。
- 已通过 SillyTavern 官方接口导入为独立角色 `魔法少女世界 0.5.122.png`。从酒馆保存文件读回确认角色版本 `0.5.122`、世界书 `魔法少女世界0.5.122`、18 条条目、7 条正则和 2 个已启用脚本；运行时含设置按钮与加载态，MVU 脚本含内置破限和 `generateRaw` 监视自愈。酒馆全局设置也已读回该角色的正则与脚本授权。

## 2026-08-27：少女日记视觉统一与字体链路修复（0.5.121）

- 从初版源码恢复 `ZCOOL KuaiLe + Noto Sans SC` 字体体系：正文和数值使用易读正文字体，标题、卡名、页签与按钮使用手写展示字体。字体导入与公共变量直接编进开始页、常态状态栏和战斗页各自的内联样式，避免酒馆助手导出 iframe 时丢失仅存在于 HTML `<head>` 的字体链接。
- 新增共享日记主题源，统一暖白纸张、粉蓝绿紫书签、虚线纸张、边框、阴影和字体回退；开始页由紫色渐变面板改为单页少女手账，常态状态栏恢复彩色书签与图钉语言。
- 战斗页保留现有横屏三段排版、战斗舞台、快速出牌和数据修复，只把深色面板统一为旧版的暖白纸张视觉；卡牌按攻击、技能、能力继续保留不同配色。生命、欲望、遗物、状态、修饰符等运行时创建的详情弹窗也由集中样式强制回到同一主题，避免点击后突然切换成旧暗色弹窗。
- 顶部 MVU 二阶段浮窗同步改为独立的少女日记悬浮球和纸张面板，仍只负责显示生成、思考、原始更新与设置，不改变主模型/MVU 的职责边界。
- 完整 `release:tavern` 回归通过，最终卡为 `魔法少女世界 0.5.121`，世界书 `魔法少女世界0.5.121`，包含 18 条世界书、7 条角色正则和 2 个已启用酒馆助手脚本。已通过 SillyTavern 官方接口导入本地 8012 实例为新角色，并启用角色正则与角色脚本。
- 最终 PNG 为 7,623,173 字节，SHA-256 `2A50B8729DA8C0E810DFCABCB82F032EA5F61E723F8BEB7A938256E206A1182E`；卡内运行时为 874,879 字节，SHA-256 `BD42564DFE3A7CD8F72D0E52B0B397AAB461382D84BA19F202A5C150F9D931AA`。Codex 可视浏览器控制仍因桌面运行环境缺少浏览器内核资源路径而无法连接，因此官方导入和数据契约已经验证，但不能把源码或静态页面检查冒充真实视觉点击验收。

## 2026-08-27：MVU 同楼修复合并与二阶段监视器（0.5.120）

- 修复“请求 AI 修复”会删除当前楼变量并从上一楼重放的问题。运行时现在保存修复前的完整当前楼，以 MVU 重放得到的“上一楼 → 修复结果”结构差异作为补丁，再合并回原快照并同步聊天变量；原更新块与修复更新块都会保留，之后重演也不会再次丢失 `battle.core.emoji`、卡组、遗物或敌人。
- 玩家与敌人的欲望效果仍要求具名对象和浅层 `effects`；运行时边界额外兼容模型偶发的扁平效果对象，自动补上通用中文名称并编译，不再因为只有操作字段而直接阻断战斗。世界书只写结构约束，不加入题材示例。
- 新增角色卡内顶部 MVU 二阶段浮窗，不依附聊天消息：自动显示生成、应用、完成或失败阶段，可实时查看兼容假流式原文和模型实际返回的 reasoning。常驻 MVU 浮球可打开设置，分别控制自动展开、原文、思考区和是否请求 thinking；默认仍关闭 thinking 以控制耗时与 token。
- 完整 `npm run release:tavern` 静默重跑用时 `148.1s` 并通过。最终角色卡 `魔法少女世界.png` 为 `7,603,429` 字节，SHA-256 `58BA7322CF18D54B08231753662D8E97C80D2BDC277357402000EB73B524209F`；角色运行时为 `867,502` 字节，SHA-256 `06F989DBDDFF6E750EFE81D70D5B1FD8CF347A8B3340EF9677AB053542D8EA77`。
- 已通过 SillyTavern 官方接口导入为独立角色 `魔法少女世界 0.5.120.png`。读回结果为内嵌世界书 `魔法少女世界0.5.120`、18 条条目、7 条正则和 2 个已启用角色脚本；MVU 脚本包含内置破限与 `generateRaw` 监视钩子，角色运行时包含顶部浮窗。真实宿主 `model-shape` 夹具使用扁平双方欲望效果通过预检，战斗页预期无修复按钮。可视浏览器控制仍因桌面运行环境无法写入浏览器内核资源而未建立，因此不把源码或静态页面检查当成酒馆视觉点击验收。

## 2026-08-27：共享展示核心、字段精度与真实酒馆导入（0.5.119）

- 卡牌页、奖励页、状态栏与战斗页现在共用 `game-core/effectDisplay.ts`。浅层效果编译、公式变量中文化、动态状态名称、目标、触发器与效果标签只维护一份；页面层只负责 DOM/CSS。源码门禁禁止 battle/common 页面重新出现独立 `switch (node.op)` 解析分支。
- AI 直接输出的战斗数值及公式常量最多一位小数；运行时逐步计算统一保留两位小数，界面最多显示一位。整数牌数、层数、能量等离散字段继续按整数处理。编译器与 JSON Schema 同时拒绝超过一位小数的直接数值，避免错误数据进入战斗。
- 奖励候选、牌堆选择、敌人意图、卡牌悬停、遗物、能力和欲望效果全部使用同一组中文效果标签；描述继续承担动作、来源与复杂规则补充，不替代可执行 `effects`。修复重复卡牌浮层，敌人欲望效果和窄屏遗物不再被隐藏。
- MVU 保持“额外模型解析 + 使用内置破限”，不创建、不读取、不切换任何玩家 preset；第二阶段生成使用 MVU 自带的“额外模型解析中”通知。开始页加入“你也可以通过直接描述一段内容来开始游戏，体验卡牌战斗内容”。
- 完整 `npm run release:tavern` 用时 `256.1s` 并通过。最终角色卡 `魔法少女世界.png` 为 `7,545,917` 字节，SHA-256 `8BB21050E7D695BA48403F68A85EA1C11FFFF75597A78DAFF67C285B6BC77A42`；角色运行时为 `847,960` 字节，SHA-256 `1BD4B715084A5FDD1F06C890878B354C018D60608E6BAEDBADA5072B83366809`。
- 最终卡已通过 SillyTavern 官方接口导入为独立角色 `魔法少女世界 0.5.119.png`，内嵌角色正则与角色脚本均启用。真实宿主已建立开始页、首轮就绪、对象式动态状态及能力/遗物触发链夹具，预检全部通过且没有修复按钮。Codex 可视浏览器控制仍因本地运行环境无法写入浏览器内核资源而不能连接；因此本版只记录官方接口与真实宿主数据链通过，不把静态页面或自动契约冒充为视觉点击验收。

## 2026-08-27：战斗舞台与整数结算（0.5.118）

- 战斗结算从保留一位小数改为最终效果统一取整；伤害、治疗、格挡、能量、欲望和生命变化不再把浮点残差写进战斗快照、界面或战后变量。MUV 战斗入口同时拒绝非整数生命/欲望，旧的带小数会话快照不再恢复。
- 战斗页新增中央行动舞台，玩家与敌人的 emoji 分居左右；攻击、技能、能力牌、事件牌、诅咒和遗物使用各自 emoji 做有界位移动画。敌人行动不再创建全屏半透明弹层，拖动出牌区也只覆盖舞台，不遮住手牌。
- `battle.core.emoji` 成为首轮必填且后续可变的玩家形象字段；只有角色变身或外观明确改变时由 MVU 更新。变量初始化、首轮生成、修复门禁、战斗请求和会话状态使用同一字段。
- 玩家能力与遗物不再叠在同一网格位置；遗物在窄宽度仍显示，敌我能力以中文名称按钮展示并可点开查看完整中文效果。遗物触发会高亮、播放舞台动画，并以遗物来源和完整触发效果进入战斗日志及战后叙事报告。
- 卡牌悬停与拖动共用唯一详情浮层，创建前同步清理旧浮层，解决出牌时两张相同详情卡叠加的问题。页面最小高度和战斗中区同步增大，同时缩短两侧标题宽度，把空间优先留给战斗内容。
- 自动回归新增 `8 × 1.1` 最终造成 9 点整数伤害、战斗输入拒绝 `46.2`、单一卡牌浮层、遗物日志、中央舞台、窄宽度遗物可见和无全屏敌人行动弹层。浏览器控制连接本轮因本地控制运行时缺失无法建立，因此没有把静态页面断言冒充交互截图。
- 完整 `npm run release:tavern` 用时 `131.9s` 并通过。最终角色卡 `魔法少女世界.png` 为 `7,498,829` 字节，SHA-256 `02DB13F5244CC3188F1CFF2FE57D9AFE2F5E2A39E25926ECB3B5DB219A7C62FA`；角色运行时为 `831,683` 字节，SHA-256 `9A593B7C7DAAA78802F3425356FCA4FF7544F00D596996120D0AD8B44A8F54AE`。
- 角色卡已通过官方接口导入隔离 SillyTavern `127.0.0.1:8012`，独立名称为 `魔法少女世界 0.5.118.png`。真实宿主先后建立开始页、合法首轮内容、模型常见对象式状态以及能力/遗物触发链聊天；全部夹具通过内容预检，最终激活聊天为 `battle-repair-triggers-2026-08-27T03-08-37Z`，没有出现修复按钮。此处只记录官方接口与真实宿主数据链验证，不宣称已完成本轮浏览器视觉截图验收。

## 2026-08-27：中文效果展示与描述职责分离（0.5.117）

- 效果标签改为从可执行效果树生成完整中文：公式变量、状态名称、目标、属性和牌区均使用玩家可读文本；伤害统一写作“对目标造成”，不再泄露状态 ID 或 `opponent.status.*` 等内部路径。
- 描述与标签分工明确。普通数值效果不再由程序自动复制成描述；AI 可省略描述，但通常应写一句中文动作、来源或表现说明。只有条件、公式、动态插牌或弃牌副效果等复杂规则缺少描述时，程序才生成中文规则兜底。
- 清理世界书中的固定卡名、敌人、题材、年代与构筑样例，保留抽象字段契约；所有玩家可见文本必须为自然中文，描述不能代替 `effects`，也不能引入未注册机制。
- 新增效果树中文标签回归测试，并扩展内容描述、适配器与世界书契约测试；正式发布卡继续使用 MVU 内置破限，不接触玩家 preset。
- 完整 `npm run release:tavern` 用时 `214.6s` 并通过；加入描述违约过滤后又重跑受影响的构建、导出、角色卡补丁与最终契约。最终角色卡 `魔法少女世界.png` 为 `7,469,661` 字节，SHA-256 `23C5D1EF39C3EE9A2793B5968830825653FD8FA52983B11409C6E483FAF0C313`；角色运行时为 `821,377` 字节，SHA-256 `1FF530BC8A572F7FA4BB2C78E11691082A33DD24A856D248FEC76399473F12DE`。卡内发布版本为 `0.5.117`。

## 2026-08-26：MVU 内置破限与模型输出容错（0.5.116）

- 按最终决定删除卡内虚拟 preset 与独立 preset 文件。MVU 首次配置直接写入“额外模型解析 + 使用内置破限”，清空“其他预设名称”；加载器不再调用、包装或等待 `getPreset/getPresetNames`，避免额外的全局接口依赖。
- 定位本次 `UNKNOWN_STATUS` 的根因：模型把 `battle.statuses` 生成为以 ID 为键的对象，而旧 MUV 边界只展开数组，导致四个完整状态定义在预检前被当成空列表。统一边界现接受规范数组、单定义对象和 ID 键对象，并可从键补全缺失的 `id`。
- 状态触发器额外容忍模型常见的单层 `{effects:{...}}` 包装；规范输出仍要求触发器直接写效果。所有状态定义继续由 AI 显式注册，程序没有新增任何内置状态。
- 为保持 AI 输出简洁，Power 仅用于打出时施加已注册持续状态时可省略 `trigger`；能力、遗物和卡牌也支持把共享 `when` 放在定义同级，编译器会把它应用于全部顶层效果，不再静默忽略条件。奖励候选复用同一 Power 规则，避免成长阶段与首轮卡组行为不一致。
- 世界书继续要求 `battle.statuses` 输出 JSON 数组且状态定义在首次引用前注册；程序容错只发生在 MUV 宿主边界，不改变 portable `ContentPack` 的数组契约。
- 加入奖励候选同规则校验并统一动态状态管理器入口后，完整 `npm run release:tavern` 再次用时 `114.5s` 并通过。最终角色卡 `魔法少女世界.png` 为 `7,462,517` 字节，SHA-256 `D11A93EF4E33BF30793332C702D6CB516BD5AC474C4526AE63C2F5C56F2707C6`；角色运行时为 `817,591` 字节，SHA-256 `637E180001E83AEE8CAB91A6AA10112433F99884EFFBD30900BD4D41F8CB0D60`。
- 官方接口导入为独立角色 `魔法少女世界 0.5.116.png`，内嵌世界书为 `魔法少女世界0.5.116`。真实 SillyTavern 保存设置已读回：内置破限、兼容假流式、自动请求、2 层历史、最多 2 次顺序请求、2600 token 与 `[mvu_update]` 白名单；卡内加载日志没有 preset 注册。
- 最终独立聊天 `battle-repair-model-shape-2026-08-26T10-45-57Z` 使用对象式状态表、触发器 `effects` 包装、持续状态 Power 和能力同级 `when`。真实战斗页正常进入玩家回合并展示敌人、手牌、能力和遗物，无修复弹窗、无 `UNKNOWN_STATUS`、“忽略无效状态定义”或战斗内容错误日志。MVU iframe 仍会在对象式和规范数组两种对照聊天中各输出相同的无路径 `Type mismatch: expected object schema but got undefined at path`，确认是当前 MVU 初始化的独立通用日志，不是状态容器分支。
...
# 2026-08-25 / 0.5.111-0.5.112 剧情模式双战闭环

- 真实聊天 `story-loop-0-5-111` 连续自然触发两场普通剧情战斗，没有依赖远征路线或限制用户输入。
- 首轮初始化生成 12 张牌、1 个遗物、1 个道具和显式注册的自定义 `burn`；第一场胜利后生命 `100 -> 86`、经验 `0 -> 25`，领取 `锻造心得` 与 `灼伤药膏` 后候选和领取限制全部清空。
- 第二场继承首战成长并正常进入横屏战斗页；胜利后生命 `86 -> 64`、经验 `25 -> 50`。第二批 3 张卡与 1 个道具候选正常生成，领取 `灼热洞察` 与 `余烬药剂` 后永久内容为 14 张牌、1 个遗物、3 个道具，候选不重复出现。
- 整页刷新会回到酒馆最近聊天页；从最近聊天重新打开后，生命 `64/100`、欲望 `4/100`、经验 `50/100`、14 张牌、1 个遗物和 3 个道具全部由消息变量恢复。最近三层窗口只保留最新需要渲染的消息状态栏，没有重复战斗 iframe。
- 实战发现状态触发器沿用普通卡牌的默认敌对目标：敌人持有的 `burn` 反而伤害玩家，玩家持有时又伤害敌人。`0.5.112` 把状态 `triggers` 的省略目标统一解释为状态持有者；显式 `to:"opponent"` 仍保留反向效果。状态仍全部由 AI 注册，不增加内置状态或额外输出字段。
- `0.5.112` 完整发布门禁用时 99.1 秒并通过；导入后世界书 `魔法少女世界0.5.112` 与角色运行时生效。真实双向状态夹具确认玩家/敌人分别持有同一状态时只伤害持有者，回合末同样分别结算；刷新重开后回合、生命、状态与牌区一致。

# 2026-08-26 / 0.5.113 开始页单页化与开放构筑预设

- 剧情模式开始页收敛为单页六项：角色名、角色形象、世界观、身份、开场时机、卡牌体系；删除阵营、偏好、模式卡、标签页和旧地点/职业选择器，不限制玩家必须填写任何字段。
- 默认提供 11 个世界观方向和 12 个卡牌构筑方向。预设只把一句自然语言填入表单，不写隐藏机制、不限制用户行动；卡牌方向只约束节奏和资源循环，具体元素、持续状态、伤害类型、名称和公式全部由 MVU 模型根据剧情现场设计并注册。
- 开始消息固定为 `mode/name/appearance/world/identity/opening/card` 浅层 JSON，只输出非空字段；世界书明确不再读取旧阵营、地点、偏好和机制字段，主模型负责剧情，MVU 模型负责初始化可执行卡组。
- UI 复用 0.2 前的紫蓝背景、白色半透明纸张、粉紫蓝强调色和少女日记式卡片语言，但没有迁回旧阵营/职业动态逻辑。补充 `srcdoc` 首个 CSS 选择器受 BOM 影响时的变量兜底，避免真实酒馆中白底、边框和文字色整体失效。
- 自动验证覆盖类型检查、浅层消息预算、至少十项预设、历史楼层锁定和世界书契约；浏览器实测桌面 1440×900 与手机 390×844 均无横向溢出，默认不填写即可开始，预设点击能正确替换文本。
- 完整 `npm run release:tavern` 用时 151.5 秒并通过；根目录最终卡为 7,445,965 字节，SHA-256：`0939DC2047E7CC5FDBA390A3FA9B80AE7396FB3A0BACCFCF51B6944D7DB352E0`。本轮结束时本机 `127.0.0.1:8012` 没有 SillyTavern 监听进程，因此只完成最终 PNG 契约与精确内联资产的桌面/手机浏览器验证，未把静态预览记作真实酒馆导入证据；恢复服务后需再导入独立名称 `魔法少女世界 0.5.113`。

# 2026-08-26 / 0.5.114 模式入口与 MVU 请求隔离

- 开始页恢复“剧情模式 / 爬塔模式”两个入口；剧情模式默认选中，爬塔模式保留可见但禁用，不影响剧情模式单页表单。
- 世界观和卡牌预设改为真正的单标题与说明，不再使用 `标题 · 副标题` 或把副标题改为空格后继续拼接。
- 卡内 MVU 加载器首次安装 `0.5.114` 时默认选择内置变量提示、只读取最近 2 层、更新请求最多重试 2 次，并固定 `[mvu_update]` 世界书白名单；只写入一次版本标记，不持续覆盖玩家后续配置。
- 新增可直接导入 SillyTavern 的 `presets/魔法少女世界-MVU变量专用.json` 及契约测试。该 preset 只启用变量执行器、`worldInfoBefore/worldInfoAfter` 和最近聊天，不带角色描述、场景、示例对话、NSFW、越狱尾注或剧情文风，避免额外模型继承主模型的续写任务。
- 角色卡默认仍使用 MVU 内置破限，因为角色卡不能替用户创建全局 preset；导入后可手动切换“使用其他预设”。首次默认同时关闭 thinking 与 Gemini 随机头部，减少无用输出和输入开销。
- 审计 MVU `onMessageReceived` 确认：单独欢迎楼层不请求额外模型；玩家提交设置后由主模型先生成剧情，MVU 在剧情回复保存时进行首轮初始化；后续也只在新 AI 回复完成后运行。“请求次数 2”是顺序失败重试上限，不是每轮固定双请求。
- 世界书按场景测量为普通更新 `3083 token`、首轮初始化 `9909 token`、遇敌注册 `9716 token`。完整卡牌、遗物、道具、状态、敌人和奖励结构不再普通轮常驻，只在初始化、内容成长、遇敌、结算、远征节点或修复标记出现时激活。
- 清理 `变量更新规则` 中仍要求逐项列出变化与原因的旧输出模板；所有变量条目和独立 preset 统一固定 `<Analysis>Update.</Analysis>`，避免第二阶段把 token 浪费在变量清单、计算过程和剧情摘要上。

# 2026-08-26 / 0.5.115 卡内 MVU preset

- 角色卡脚本现在把变量专用 preset 直接内嵌并注册为 `魔法少女世界卡内变量预设`，不再要求用户手动导入全局 preset。
- 注册采用命名空间包装：原有 `getPreset/getPresetNames` 继续委托，只有卡内 preset 名称返回变量执行器；不会切换、覆盖或持久化酒馆现有剧情 preset。
- 默认 MVU 配置改为“使用其他预设 + 卡内变量 preset”，并固定 `聊天消息`、`依次请求，失败后重试`、2 次、2 层历史、2600 token、关闭 thinking、关闭随机头部。假流式迁移标记同步写入，避免 MVU 自身迁移在加载后把它反转为 false。
- 如果旧版酒馆助手没有 preset API，卡内加载器最多等待 5 秒后自动退回 MVU 内置破限；不会因为可选隔离增强不可用而阻塞角色卡。
- 版本升级为 `0.5.115`，确保已导入 `0.5.114` 的酒馆用户能够执行一次新的卡内配置初始化。
- 完整 `npm run release:tavern` 通过；随后针对最终单标题与 Analysis 清理再次通过类型检查、开始页/世界书/preset 契约、生产构建、酒馆运行时导出、角色卡补丁和 `verify:tavern`。桌面 `1440x900` 与手机 `390x844` 浏览器复核均为 11 个世界观、12 个卡牌方向，剧情模式默认选中、爬塔模式禁用、空表单可开始，页面无横向溢出或标题裁切。
- 完整 `npm run release:tavern` 已重新通过。最终角色卡 `魔法少女世界.png` 为 `7,460,901` 字节，SHA-256 `12A8D6ADE290BB4985680058AA9FA5B1505DF7C7682143D3E5114F28DB82C4A1`；角色运行时为 `814,871` 字节，SHA-256 `8321EF9ECE1E8081AA66B30E74D08DB40C7934162BDE07F6E65C21EA9866D0B5`；MVU 专用 preset 为 `2,197` 字节，SHA-256 `C63BEEA6211E288EFDC2BC81FBC9E33EF8AD09052C5596CE751BBAC4F01DD22A`。最终 PNG 内为角色 `魔法少女世界 0.5.115`、世界书 `魔法少女世界0.5.115`、18 条世界书条目、7 条正则和 2 个角色脚本，并确认 MVU 脚本内含卡内 preset 与内置破限回退。本机 `127.0.0.1:8012` 仍无 SillyTavern 监听，因此本轮不把静态预览冒充成真实酒馆在线生成；恢复酒馆后仍需导入独立名称 `魔法少女世界 0.5.115` 完成最终模型请求复核。
- 本地真实酒馆安装目录为 `D:\project\_codex-tavern-e2e`，版本 `1.18.0`；测试数据不使用安装目录内的默认 `data`，而是保存在 `D:\project\_codex-tavern-data-selfcontained`。恢复时必须在安装目录执行 `node server.js --port 8012 --dataRoot D:\project\_codex-tavern-data-selfcontained --browserLaunchEnabled false`；`npm run dev:tavern` 和端口 `5500` 只提供项目静态 iframe 资源，不能代替 SillyTavern 主服务。
- `0.5.115` 已通过 SillyTavern 官方导入接口导入为 `魔法少女世界 0.5.115.png`，并创建独立聊天 `start-runtime-latest-2026-08-26T07-49-13Z`。浏览器确认内嵌世界书 `魔法少女世界0.5.115` 成功导入并链接，共 18 条；真实消息 iframe 完整渲染开始页，包含默认剧情模式、禁用的爬塔模式、11 个世界观方向、12 个卡牌方向和可直接开始的空表单。酒馆保存的 MVU 配置已读回为“额外模型解析”“使用其他预设”“魔法少女世界卡内变量预设”、兼容假流式、自动请求、与插头相同、2 层历史、最多 2 次顺序重试、2600 token 和 `^\\[mvu_update\\]` 白名单，卡内默认版本标记为 `0.5.115`。
# 2026-08-28：变量更新改为楼层独立 HTML（0.5.132）

- 将角色卡原有的“去除变量”正则原位升级为“变量更新展示”，不增加角色正则数量。`<UpdateVariable>...</UpdateVariable>` 不再被静默删除，而是在对应消息楼层渲染为独立的少女日记风格 HTML。
- 新页面从当前楼层原始消息读取并解析 `_.set`、`_.assign`、`_.remove`、`_.add`，按角色与剧情、战斗核心、卡牌、遗物、道具、能力与状态、敌人、人物与势力、奖励分组。对象、数组、三参数 `_.set` 以及字符串中含 `);` 的命令均走同一解析器。
- 卡牌、遗物与道具继续复用游戏核心的公式中文化和效果标签，不在新页面重复实现一套公式解释；内部模式锚点、远征状态和一次性请求字段不向玩家展示。
- 普通常态状态栏移除旧“状态更新/本次变化”区域，避免同一变化重复显示；战斗正则不再吞掉变量块，因此一个战斗楼层可同时显示变量更新 HTML 与战斗页面。变量更新页默认只作用于最近三层，降低长对话中的前端负担。
- 最终 `0.5.132` 已通过 SillyTavern 官方接口导入为独立角色，读回为 20 条世界书、7 条正则和 2 个已授权角色脚本。真实聊天同时验证普通楼层的 `update + common` 与战斗楼层的 `update + fish`；资源亲和者样例 10 项完整显示，战斗页读取玩家 `100/100`、敌人 `41/48`。最终卡为 `7,938,325` 字节，SHA-256 `9F39A4E734F7433B742808B17971B0E821C4EB0005AB3AB9055C5159A3AAD0A3`。

# 2026-08-28：战斗交互与战后剧情交接（0.5.133）

- 战斗 HUD 在保留“敌人 / 手牌 / 我方”结构和新增战斗舞台的前提下，恢复 0.2 前星空战斗语言：放大生命与欲望条，状态默认只显示 emoji，遗物、能力、状态与欲望效果统一为可点击详情托盘。
- 桌面拖牌不再使用浏览器原生 HTML 拖放。角色卡以 Pointer Events 独立接管单张手牌，显示随指针倾斜的卡牌副本、循环光泽、投放区反馈、取消返回和成功飞向战斗舞台；实际出牌不等待表现动画，避免牺牲操作速度。
- 战斗历史改为结构化数据并写入战斗快照，最多保留 600 条诊断事件。剧情交接按回合压缩玩家出牌、敌人行动、遗物/能力/状态触发和关键事件；同回合重复行动显示为“名称×次数”，完整卡组、遗物、能力、道具、欲望效果和最终敌我属性另外汇总，不再把逐条伤害日志或奖励模型指令发给剧情模型。
- 战斗结束页改为星空暗色弹窗，胜利、失败、终止分别驱动标题光晕、徽标、输入焦点和确认按钮颜色。新增最多 500 字的可选战后行动输入，确认后与回合摘要一同发送；发送失败时保留输入并恢复按钮。
- 自动回归覆盖回合摘要完整性、重复行动计数、战斗快照恢复、敌我动画锚点、战后输入交接、胜负色样式、类型检查和生产构建。真实酒馆导入、拖牌和最终发送内容仍以本版本验收记录为准。

# 2026-08-28：战后构筑机制与旧快照能力来源（0.5.134）

- 战后剧情交接继续按回合压缩操作，但“完整构筑与资源”不再只写名称和剧情描述：卡牌、遗物、能力、道具、敌方行动及双方欲望效果统一复用游戏核心公式中文化，发送来源、实际效果和有界说明。机制完整保留，重复牌按数量合并，说明限制长度以控制 token。
- 旧战斗快照中只有 `ability__N` 的动态能力会按触发器与效果程序反查来源卡牌、遗物或敌方行动，恢复可读名称、emoji、描述和来源；无法可靠反查时明确标为临时能力，不伪造来源。
- 真实酒馆验收补充精简夹具模式：只保留目标战斗楼层与变量快照，避免长剧情和历史 iframe 干扰拖牌、结算弹窗与发送链路测试。另支持胜利、失败、终止终态夹具，无需模型请求即可检查三套配色。

# 2026-08-28：战斗 UI、回合战报与三色结算收尾（0.5.136）

- 删除战斗页末尾重复覆盖布局，收敛为一套 `640px` 高星空界面。敌方、无文字战斗舞台、手牌、我方和底部操作区在同一屏展示，状态、能力、遗物和欲望效果继续通过详情弹层展开。
- 桌面出牌继续使用 Pointer Events 的单卡拖动表现；加入 `420ms` 拖牌副本清理保险，避免个别酒馆环境中 `Animation.finished` 永久 pending 后残影和拖动状态不消失。真实出牌确认能量、抽牌和牌区正常，页面中没有遗留 `.card-ghost` 或 `.dragging`。
- 战后文本改为按回合计数的紧凑摘要，合并同回合重复出牌并记录敌人行动、遗物、能力和状态触发；完整构筑、来源、双方终态及牌堆另行汇总，不发送逐条伤害流水或奖励模型内部指令。
- 结算弹窗支持最多 500 字的可选玩家行动，确认后通过 `playerContinuation` 与战报一并交给剧情模型。胜利、失败、终止分别使用绿、红、灰蓝主题。
- 真实 SillyTavern `1.18.0` 中分别打开三个无模型请求的终态夹具。519px 宽消息 iframe 无横向溢出，三种面板、输入框和按钮均完整；结果色读回为 `rgb(76, 175, 80)`、`rgb(244, 67, 54)`、`rgb(84, 110, 122)`，输入 20 字后计数器正确显示 `20/500`。
- 官方导入读回为角色 `魔法少女世界 0.5.136`、世界书 `魔法少女世界0.5.136`、20 条世界书、7 条正则和 2 个角色脚本。最终卡 `魔法少女世界.png` 为 `7,978,117` 字节，SHA-256 `DEF1A63B2E49B7FE546C67F1B46F44BAF20D00F3E9B4C24E7DE449FABAFCAE8B`；本轮保持未提交、未推送。

# 2026-08-28：战后输入与触发记录补全（0.5.138）

- 战后面板继续使用星空暗色与胜利/失败/终止三色结果强调；500 字可选行动会在用户确认后重建最终战报，与完整回合摘要通过同一条剧情继续消息发送。
- 补齐能力与状态的成功触发来源，与遗物共用结构化战斗历史；同回合重复触发改为“名称×次数”，不恢复逐条伤害流水。
- 回归测试现在直接调用战后面板 `onConfirm`，确认玩家指定行动确实进入剧情模型提示；同时覆盖卡牌、敌方行动、遗物、能力和状态的回合计数。
- 桌面拖牌仍由 Pointer Events 单卡副本实现跟手、倾斜、投放高亮、成功飞出和取消回弹；HTML `draggable` 持续强制为 `false`，防止整组手牌被原生拖出。
- 完整 `npm run release:tavern` 用时 231.5 秒并通过。官方导入读回为角色 `魔法少女世界 0.5.138`、世界书 `魔法少女世界0.5.138`、20 条世界书、7 条正则和 2 个角色脚本。最终卡 `魔法少女世界.png` 为 `7,984,621` 字节，SHA-256 `9F63E75AEE20E3C765FFD7DCCBE5A557EDEA6012E57062D389A82FFE18357D72`；本轮仍未提交、未推送。

# 2026-08-28：实体卡拖动与牌区可读性收尾（0.5.139）

- 删除桌面拖牌的副本、半透明虚影和 `clone()` 路径，改为把当前实体卡临时提升到 viewport 层。移动事件由 `requestAnimationFrame` 合并，使用 `translate3d` 跟手；成功投放立即执行出牌，取消时把同一张实体卡送回原位。卡牌详情跟随同一位移，不再逐帧触发布局测量。
- 牌堆入口从纯数字改为“牌组 / 抽牌 / 弃牌 / 消耗 + 数量”；完整牌组弹窗改为自适应网格，弹窗卡牌不再使用手牌的绝对定位，不会叠成一张或横向裁切。效果说明继续复用统一中文效果渲染。
- 欲望效果从与状态混排的 emoji 改为“欲望效果：名称”，点击名称展开所属方、完整描述和实际效果。变量更新楼层改为默认折叠，玩家需要时再查看完整变化。
- 删除构筑偏好型报错 `NO_DEFENSE_OR_RECOVERY`、`NO_DEFENSE` 与 `SMALL_DECK`。纯攻击、连击、特殊胜利或无回复构筑不再被程序判错；仍保留能量可支付、战斗可结束、敌人有实际压力及字段/效果程序合法性等运行必要校验。世界书同步明确不强制格挡、治疗或固定攻防比例。
- 完整 `npm run release:tavern` 用时约 323 秒并通过；官方导入读回为角色 `魔法少女世界 0.5.139`、世界书 `魔法少女世界0.5.139`、20 条世界书、7 条正则和 `MVU变量框架`、`魔法少女世界运行时` 2 个角色脚本。最终卡 `魔法少女世界.png` 为 `7,991,901` 字节，SHA-256 `ABF200FA2440AC471D014A727AE2B3C07D70E0B36DA5169861CE453E1208B765`。
- 在真实 SillyTavern `1.18.0` 的 519px 宽消息 iframe 中使用无模型请求的 active 夹具验收：拖出“灰爪撕咬”后敌方生命 `100/100→88/100`、能量 `3/3→2/3`、手牌 `6→5`、弃牌 `2→3`，没有 `.card-ghost` 或残留 `.dragging`。13 张完整牌组按网格显示；双方欲望效果名称可点击并显示描述和实际效果。本轮保持未提交、未推送。

# 2026-08-28：统一出牌交互与结算事务防火墙（0.5.140-0.5.141）

- 拖动与连续点击不再是互斥模式：第一次点击只选中卡牌，第二次点击同一张牌才出牌；拖动始终同时可用，删除模式切换按钮和本地模式状态。
- 拖动改为“不可见原槽 + 唯一实体卡”结构。实体卡离开手牌后原槽继续参与扇形布局；取消时直接插回槽位，不记录返回坐标、不创建副本、不播放几何回弹。新增战斗全屏按钮，优先请求原生 iframe 全屏，失败时使用可恢复的 viewport 覆盖模式。
- 长流程审计发现 `0.5.138` 的战败二阶段虽然登记了永久状态，却同时重新写入已清理敌人并清空战斗道具，导致最终验收没有完成。`0.5.141` 新增最小数据所有权保护：生命、经验、敌人和已结算道具数量以程序快照为准；除此之外不设置路径白名单，不限制剧情模型继续改变构筑、人物、物品或世界状态。
- 胜利现在允许伴随正文已经确认的持久代价；战败允许按严重程度生成一项或多项惩罚，并可组合诅咒牌、负面遗物和永久状态。新增纯事务测试和角色运行时事件测试，覆盖确定数值不被重复结算，同时确认多项创意后果不会被程序回滚。浏览器人工验收按用户要求交由用户执行；本轮不将未进行的浏览器操作记为通过证据。
- 修复角色运行时误引入模块后导出为 CommonJS、在酒馆经典脚本环境触发 `exports is not defined` 的问题；结算保护保持在唯一的卡内运行时实现中。最终 `0.5.141` 单卡为 `8,013,133` 字节，SHA-256 `84408FD05BB69052496234861D3B6AEC8D7D6B8A7A67DE95FFD654CC4A1DE20A`；结构、运行时和嵌入契约通过，未执行浏览器人工游玩验收。
- 新增不调用模型和浏览器的双战确定性长期流程，连续覆盖初始化、首战胜利、带代价的奖励生成、领取、继承成长后的第二次遇敌，以及诅咒牌、负面遗物和多项永久状态混合战败惩罚。点击与拖动结果、实体卡原槽恢复、全屏 fixed viewport 覆盖及宿主样式还原分别收敛为单一可执行策略并纳入发布门禁；完整证据见 `docs/story-mode-acceptance-0.5.141.md`。

# 2026-08-28：真实双战验收与欲望满溢防重入（0.5.142）

- 在真实 SillyTavern 与真实额外模型中连续跑通初始化、普通剧情轮、第一战、三张卡牌与一个道具候选、领取成长、第二次遇敌、战败惩罚和战败后继续剧情。真实请求与状态快照保存在 `real-story-loop-0.5.141.log`，证明剧情模式主线可以跨多轮成长而不反复初始化卡组。
- 长流程发现第二次遇敌时模型会尝试增加 `battle.exp`。世界书现明确规定等级和经验只允许首次初始化时写入，后续完全归程序结算；真实流程脚本增加断言，第二次遇敌或战败阶段篡改等级、经验会直接失败。该约束只保护程序所有权，不限制剧情确认的构筑变化、胜利代价或多项战败惩罚。
- 真实 UI 复现“结束回合后页面卡死”：玩家欲望满溢触发敌方欲望效果，而该效果再次增加欲望，旧实现会在清零前递归触发自身。统一效果执行器增加按目标防重入锁，并在 `finally` 中归零与释放；即使欲望效果自身含欲望变化也只结算一次。
- 真实鼠标事件已确认双击只出一张、拖动只出一张、取消拖动保持实体卡 ID 顺序并回到原槽，未残留拖动虚影或选中态。新增生产方法级回归让玩家/敌人欲望效果在自身再次触发满溢，均只执行一次；成功与异常路径都归零并释放锁。恢复到战后成长阶段只重跑一次第二次遇敌额外模型，卡组保持 13、等级 1、经验 25，敌人通过预检且没有模型越权经验命令。
- 完整 `npm run release:tavern` 用时约 456.5 秒并通过；官方导入读回为角色 `魔法少女世界 0.5.142`、世界书 `魔法少女世界0.5.142`、20 条世界书、7 条正则和 2 个角色脚本。最终单卡为 `8,014,637` 字节，SHA-256 `42E2C300A965FD890E10A9B2129211D691D93558DC453B7C3D81DAC30372AF9A`；本轮保持未提交、未推送，完整结果见 `docs/story-mode-acceptance-0.5.142.md`。

# 2026-08-28：MVU 原楼层修复事务收敛（0.5.143）

- 定位附件中的双 `<UpdateVariable>`：修复宿主错误地把原始无效块重新插回楼层；同时未屏蔽 `<CHARACTER_INIT_PENDING>`、`<CONTENT_PENDING>`、`<BATTLE_PENDING>`、`<BATTLE_START>` 与结算/远征锚点，使额外模型在修复时再次执行初始化和遇敌生成。
- 修复请求期间只保留 `[战斗内容修复]` / `[战斗场景修复]`，临时隐藏其他生成锚点；成功后以最新修复块替换楼层内全部旧更新块，原剧情正文、状态占位符和运行标记按原样保留。最终楼层只允许一个 `<UpdateVariable>`。
- 修复结果先与本楼层原快照合并，再执行完整战斗预检；仍含非法弃牌对象、函数公式、空状态子触发器或其他错误时，消息变量、聊天变量和楼层正文全部回滚，不再把“变量有变化”误判为成功。每类紧凑错误路径上限由 4 提高到 8，覆盖一次回复中的多项独立错误。
- AI 契约明确 `discard/exhaust` 的值不可为对象，`from/pick` 必须同级；主动弃 1 张也必须显式写 `discard:1`，不适用普通字段“数量为 1 可省略”的规则。弃牌、抽牌、消耗和回收等牌区操作必须独占 `effects` 数组项，不能与伤害或其他牌区操作混写。公式禁止函数；状态仅可让根 `triggers` 为空，子触发器不得为空对象/数组。修复规范要求逐项处理全部 `问题=` 路径，并禁止借单项修复重建未涉及的永久内容。
- 同步清除世界书中残留的强制防御/治疗要求：仍校验基础能量可出牌、可靠胜利手段和可执行性，但攻守与恢复比例继续由自定义构筑决定。
- 完整 `npm run release:tavern` 用时约 257.5 秒并通过。官方接口和浏览器实际导入均确认角色 `魔法少女世界 0.5.143`、世界书 `魔法少女世界0.5.143`、20 条世界书、7 条正则和 2 个已授权角色脚本；开始页正常渲染且 MVU 初始化成功。最终单卡为 `8,023,445` 字节，SHA-256 `5369ADBE1F82E27A782B9730BF0B0963F2D2B3F0D777DDA85E920ED073215FA3`；本轮保持未提交、未推送。

# 2026-08-28：条件、触发与弃牌后效果说明统一（0.5.144）

- 修复固定数值 `discard_effects` 不会进入描述字段的问题。只要卡牌存在弃牌后效果，无论是否包含公式，都会明确显示“此牌被战斗效果弃掉后”以及实际结果。
- `when`、`on`、结构化 `trigger`、分支和弃牌后效果统一由可执行字段生成权威中文规则。比较符改写为“高于、低于、不低于、不高于、等于、不等于”，常见二分之一条件直接显示为“一半”，不再向玩家展示 `if`、`>` 或内部变量路径。
- AI 的动作、来源和质感描述继续保留；若 AI 叙事复述了数值或与可执行规则冲突，程序丢弃该机械复述并使用编译结果重建说明。普通状态栏、奖励、牌堆、手牌详情和战斗悬浮提示复用同一语义。
- 世界书要求所有带条件或触发时机的复杂内容明确说明“何时触发、满足什么条件、发生什么”，但不加入固定题材或具体构筑范例。
- 完整 `npm run release:tavern` 用时约 282.3 秒并通过。官方接口读回角色 `魔法少女世界 0.5.144`、世界书 `魔法少女世界0.5.144`、20 条世界书、7 条正则和 2 个已授权角色脚本，且已设为本地活动角色。最终单卡为 `8,039,797` 字节，SHA-256 `6C1D261DE71164B6E5FC85F093BEFE84D15630AD9FAD028F7931DEF21EC05B89`；本轮保持未提交、未推送。
# 2026-08-28：剧情驱动敌人机制与全屏入口说明（0.5.145）

- 全屏入口改为明确的“全屏游玩 / 退出全屏”，补充占满当前窗口与返回酒馆楼层的无障碍说明；小屏不再隐藏按钮文字。
- 删除“欲望主题敌人必须以欲望压力为主、生命伤害为辅”的硬约束。敌人机制改由当前剧情身份、形态、行为和场景决定，欲望只是可选组合，不预设压力比例。
- 普通敌人可在剧情需要时使用少量完整注册的自定义状态、特色能力、临时牌或牌库干扰；同时要求可执行终局路径、合理应对窗口和适度复杂度，避免纯数值堆叠与无效机制堆砌。
- 初始牌组不再因总 `quantity` 超过 13 报错或被运行时判定为未初始化；生成与修复世界书同步取消总量上限。结构、费用、可执行效果、状态引用、重复 ID、基础可打出性和胜利路径仍由既有契约校验。
- 完整 `npm run release:tavern` 用时约 289.1 秒并通过。官方接口读回角色 `魔法少女世界 0.5.145`、世界书 `魔法少女世界0.5.145`、20 条世界书、7 条正则和 2 个已授权角色脚本，且已设为本地活动角色。生产战斗页在 519×740 视口中“全屏游玩”文字可见且无横向溢出。最终单卡为 `8,045,453` 字节，SHA-256 `6926F3FFA3625E2DC6DE139DF769E5B96D6ABD081FCBCE6DA96D2FD998B561F2`；本轮保持未提交、未推送。

# 2026-08-28：开始页常用预设重写（0.5.147）

- 上一版卡牌与敌人生成知识库未达到体验预期，已从世界书、发布链和角色卡中撤下，后续重构事项记录在 `TODO.md`；现有研究仅保留为维护者候选资料。
- 世界观与卡牌预设全部重写。世界观使用 12 个常见题材，卡牌使用 12 个常见构筑方向，不再保留白木市、固定事件、预设职业或过长机制说明。
- 预设卡片改为紧凑自适应布局；标题与一句说明直接展示，选中标记不再占用文字排版空间。
- MVU 二阶段浮窗改为请求开始时创建计时器，成功、失败或超时后清理并冻结最终耗时，避免完成状态持续计时。
- 角色卡补丁器不再只做世界书条目覆盖；输出前会按 manifest 重建条目列表，防止旧 PNG 中已删除的实验条目混入新卡。
- 完整发布链通过后，以新显示名 `魔法少女世界 0.5.147` 经官方接口导入本地 SillyTavern；读回世界书 `魔法少女世界0.5.147` 共 20 条、7 条正则和 2 个已授权角色脚本。开始页实机显示 12 个世界观与 12 个卡牌预设，内嵌世界书提示确认后可正常导入。最终单卡为 `8,045,045` 字节，SHA-256 `354ECAE8413FB4D0F9DF5CCAA9B6DC720BEB73B5C7B5EE4879D033133876A8C8`；本轮保持未提交、未推送。
