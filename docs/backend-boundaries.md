...
自2026-09-18用户明确要求起，项目以最新版本的公开协议、内容和存档结构为目标。1.0.2之后的下一版本清理不再被最新流程使用的旧协议解析器、历史字段别名、旧存档迁移和兼容执行分支；不因旧测试或历史文档仍存在就继续维护它们。对外入口服务当前版本，不承诺历史调用形状永久可用。

删除前核对最新默认入口的实际调用及数据来源，并同步 Prompt、Schema、编译、校验/有限修复、运行时、保存恢复和双说明链。最新合法内容的规范化、事务回滚、损坏数据拒绝和当前支持环境的适配仍是必要能力。旧数据不受支持时明确提示，保留原始数据，不做猜测转换或自动覆盖。
