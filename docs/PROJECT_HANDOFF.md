> **2026-09-23 1.0.4 已正式发布并本地安装。** 正式版本及扩展、卡图仓库远端均已同步，公开附件与卡图 SHA-256 读回通过；本地酒馆已新增 **魔法少女世界 1.0.4**（头像 **魔法少女世界 1.0.4.png**）、关联世界书与 1.0.4 扩展，原 1.0.3 仍保留且仍为当前选中角色。玩家自行选择新卡并完整刷新后再新开局；未做真实模型生成与浏览器实玩验收。详情见 [1.0.4 发布记录](tasks/2026-09-23-1.0.4发布.md)。

> **2026-09-23 替代费用／预设状态预览已本地安装**：卡牌费用统一改为「可选替代：方案名，费用」，删去冗余「（替换全部通常费用）」。对于会话 `battle.statuses` 未登记但卡牌引用内置 `sts_strength` 的情况，中文说明及独立预览可解析为「力量」及其执行规则，保留同 ID 自定义状态优先、未知状态仍提示未注册；不改已有玩家存档。双类型、专项/DOM 回归与隔离构建通过；备份安装最新 **魔法少女世界 1.0.3**（头像 **魔法少女世界 1.0.3.png**），运行时读回 SHA-256 `1c88141784e4bdb8457ac7e1513e70f2c30ecd8b6094a2344ce58a75bfbc4b8f`，1127 份非目标用户文件未变。玩家需自行完整刷新页面；未做真实模型、浏览器视觉和实玩验收。见 [费用与状态预览记录](tasks/2026-09-23-payment-status-preview.md)。

> **2026-09-23 本轮 MVU 明确拒绝已查明并改进提示**：新诊断关联到当前请求的助手完成事件明确拒绝且没有更新块；楼层解析仅是剧情回退，原始网络响应仍未保留。仅改监视器错误分类与回归，无自动重试或变量写入；两套类型检查、专项测试及隔离构建通过。已备份安装最新 **魔法少女世界 1.0.3**（头像 **魔法少女世界 1.0.3.png**）运行时，读回哈希与隔离产物一致，其余 359 个玩家文件未变。玩家需自行刷新并以合规剧情重试；未做真实模型／浏览器实玩验收。见 [诊断与安装记录](tasks/2026-09-23-mvu-explicit-refusal.md)。

> **2026-09-23 刷新后变量生成仍有失败，诊断关联已修订并本地安装**：新导出 14:52:45（旧附件仍为 14:19:55）；刷新后两次成功、一轮 14:49:44–14:51:31 失败。失败诊断的 1858 字符助手完成事件早于最后一次 MVU 请求，3180 字符楼层解析仅是回退、不是提供方原始响应。现修正跨请求完成输出的归属、晚到事件和双载体提示词摘要；源码两套类型检查、专项回归与隔离构建通过，仅更新最新 **魔法少女世界 1.0.3**（头像 **魔法少女世界 1.0.3.png**）的卡内运行时，安装内容 SHA-256 `f768f03e5b7d87c4563e391516c41be9f169724f7ae7bd0557813ed8e397f09e`，其他 360 个受保护角色／聊天／世界书／设置文件哈希不变。**这并未证明生成失败根因或修复模型输出**，用户需自行刷新并实测；没有自动生成、推进聊天或刷新页面。详见 [本轮诊断与安装](tasks/2026-09-23-mvu-request-final-association.md)。

> **2026-09-23 变量生成连续失败的现场接续（尚待用户刷新复测）**：14:11:36 开始的这轮 MVU 请求于 14:12:28 报未见 `<UpdateVariable>`；所附 2992 字符回退文本与保存的剧情楼层一致，不是可确认的提供方原始响应。诊断仍显示旧“模型返回完成”标签且缺少新版请求摘要字段，说明当次浏览器页面在使用旧内存脚本；安装在卡内的新运行时、七条世界书及扩展 5 文件 HTTP 已读回核对。14:21 检查发现测试酒馆 8012 无监听，已改用独立隐藏进程重启，禁用自动开浏览器，HTTP 200。**不能据此断言每次失败均由服务中断或模型造成，也未擅自刷新、生成或改存档**；需用户自行完整刷新后重试一次，失败时提供新诊断。详见 [变量生成现场记录](tasks/2026-09-23-variable-generation-stale-page-server.md)。

> **2026-09-23 本地测试酒馆已启动并完成两轮待安装修订**：测试酒馆现运行于 `127.0.0.1:8012`（启动时禁用自动打开浏览器）；隔离候选 `preset-scope-20260923-r1` 的 1035 份源文件、18 份构建产物安装前后哈希一致。只更新最新 **魔法少女世界 1.0.3**（头像 **魔法少女世界 1.0.3.png**）的卡内运行时、测试扩展 5 文件和关联世界书 `魔法少女世界1.0.3` 的条目 0、1、2、6、9、10、17 正文；关联／嵌入／原始世界书、运行时 API 及扩展 HTTP 哈希读回通过。设置、全局正则、选中聊天及 356 份受保护的其他角色／聊天／世界书／设置文件未变。备份与核验范围见 [安装接续记录](tasks/2026-09-23-preset-status-authoring-scope.md)；没有刷新页面、生成内容、推进聊天或真实模型／浏览器实玩验收，需用户自行完整刷新酒馆页面。

> **2026-09-23 预设状态使用范围已收紧（原源码阶段；现已本地安装，见本页顶条）**：世界书完整保留15项预设并补齐11增益/4减益分类；新生成仅允许无剧情身份、机制简单的普通杂兵或偏中立卡牌使用，剧情相关敌人、复杂敌人和玩家整套卡组必须按剧情设定具体创作，不得套预设或只换名。七份世界书、生成/修复 Prompt 和 Schema 描述同源；不增加质量硬门槛，不改既有合法内容或存档。双类型、专项契约及隔离候选 `preset-scope-20260923-r1` 通过，1035源码冻结、1031保护文件未变。候选包含上轮当时尚未安装的 MVU 诊断修订；源码验证时 8012 测试酒馆未启动，随后已按本页顶条完成安装。目标显示名 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**；未刷新/调用模型/推进聊天，无真实模型或实玩验收。详见 [预设状态创作范围](tasks/2026-09-23-preset-status-authoring-scope.md)。

> **2026-09-23 连续生成失败／预设 Prompt 核查（原源码阶段；现已本地安装，见本页顶条）**：两份附件只证明 MVU 未得到更新块，第二份回退正文与原剧情楼层相同，均无提供方原始响应；上游失败根因仍待取证。已确认当前 1.0.3 世界书包含灵巧、遗忘及 15 项预设状态，但七个条目的旧“没有内置状态”说法冲突；本次统一白名单及请求/修复/Schema 提示，补助手完成输出与楼层解析的区分。双类型、专项回归及隔离候选 `mvu-prompt-audit-20260923-r1` 通过，原 dist/已安装扩展 1031 文件未变；旧 tower-request 文案断言失败已在 HEAD 复现。源码验证时测试酒馆 8012 未启动；本任务随后随最新隔离候选 `preset-scope-20260923-r1` 一并安装，见本页顶条。目标卡显示名 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**。未刷新/生成/推进聊天，未真实模型或实玩验收。详见 [失败诊断与 Prompt 核查](tasks/2026-09-23-generation-failure-prompt-audit.md)。

> **2026-09-23 新增流派已补齐并安装本地测试酒馆（当前入口）**：前次状态复制／转移仅混在“状态净化”，额外／替代支付与状态伤害前／出牌前拦截未作为独立选项；世界书流派条目虽存在但沿用旧引导。提交 `3f8da60` 新增六个可选基础机制（合计 144 项）、准确的结构识别与世界书设计链。隔离构建与安装前冻结核验通过；更新本地最新测试角色 **魔法少女世界 1.0.3**（头像 **魔法少女世界 1.0.3.png**）的运行时、测试扩展和世界书 ID 21；三处世界书回读与扩展五文件 HTTP 哈希一致，未刷新或操作玩家聊天。用户须自行完整刷新页面；真实模型／实玩仍未验收。详见 [流派表与世界书补同步](tasks/2026-09-23-archetype-catalog-worldbook-sync.md)。
> **2026-09-23 第二幕开局馈赠卡死已修复并安装本地测试酒馆（当前入口）**：提交 `5c354b9`、`d1885e7`。Boss 遗物逐项领取留下零额度候选造成旧存档馈赠拒绝；现在新领奖完成时清理未选项，已卡住的存档在馈赠事务中仅清理不能领取的残留，并保留已领取卡/遗物/金币；真正未领取奖励需先处理。`act2-opening-20260923-r2` 隔离构建，运行时及扩展 5 项 HTTP 回读一致；没有改写聊天或刷新页面。应测试 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**，自行刷新完整酒馆页；真实模型/玩家聊天未验收。详见 [第二幕馈赠修复与证据](2026-09-23-act2-opening-reward-recovery.md)。

> **2026-09-23 机制缺口已提交并安装本地测试酒馆（当前入口）**：状态选择／复制／转移在前次 `f9fe60f`；本次 `acc7e80` 补全卡牌额外／替代费用与通用结算前拦截，更新世界书、Schema、两条说明、修复契约、事务、存档与交互。候选 `mechanisms-20260923-r1` 隔离构建且原 dist/扩展 1031 文件哈希未变；69 操作／114 分支、32 组稳定性、双类型、390/1000 隔离浏览器和专项契约通过。角色运行时及 5 项扩展 HTTP 哈希一致，世界书 ID 5/1 回读一致；选中聊天不变，未刷新或推进存档。应测试显示名 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**，用户自行刷新完整酒馆页。真实模型和玩家实玩仍未验收。详见 [机制收尾与安装证据](tasks/2026-09-23-sts2-mechanism-completion.md)；下方“尚未实现”属于安装前历史记录。

> **2026-09-22 卡牌绑定权限已本地安装（当前入口）**：源码 `bfd94b7`、`d12a2ba`，候选 `permissions-20260922-r1`。新增独立不可永久移除/不可变形，统一剧情、商店、营火、事件、馈赠与战斗；遗忘/销毁遇保护只退出本场，日志和评分同步。29组稳定性、5组定向、双类型和390/1000隔离商店浏览器检查通过；卡内运行时与5项扩展HTTP哈希一致。351份保护基线中350份未变，另一聊天在安装前5分钟已更新，保留差异且未改写；选中聊天安装前后不变。测试显示名 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**，用户自行刷新完整酒馆页。尚未实现状态交互选择/转移、额外或替代费用、通用结算前拦截；不代表四项缺口均已补完。详见 [机制补完与安装证据](tasks/2026-09-22-mechanism-completion.md)。

> **2026-09-22 流派与评分首批修复已本地安装（历史基线）**：源码 `5859abd`，候选 `scoring-20260922-r1`。修复评估缓存身份、欲望刷清敌进度、灵巧/零费漏识别、目标/因果误判、基础牌阈值断崖、无关状态加分与资源初始可用性；敌人周期按实际序列，被动未完整模拟时降低覆盖并说明。无伤害欲望对照从100%回到35%，96次正式试打、28组稳定性、15组定向检查及双类型通过；1027源码冻结、1031构建保护文件未变，卡内运行时/5项扩展HTTP哈希通过，348份其他角色/聊天/世界书未变。测试显示名 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**，用户自行刷新完整酒馆页。未调用真实模型或进行玩家实玩/新视觉验收。后续优先状态选择/转移、额外或替代费用；通用结算前拦截/永久移除限制尚未实现，已有缓冲/人工制品/无实体不能重复算缺失。详见 [修复、缺口与安装记录](tasks/2026-09-22-scoring-fixes-mechanism-gaps.md)。

> **2026-09-22 流派与评分审查完成（历史审查基线）**：核对 67 个流派节点、138 个基础预设及静态/影子/正式引擎评分调用链。26 个合法合成包复现缓存 ID 碰撞、词条漏识别、目标/因果误判、基础牌阈值跳变、无关状态加分和敌人周期/被动估分偏差；96 次正式引擎试打证实无伤害欲望循环可在 0 胜时取得 100% 表现分。8 组现有回归通过；后续首批修复见上条。详见 [审查、复现与方案](tasks/2026-09-22-archetype-scoring-audit.md)。
