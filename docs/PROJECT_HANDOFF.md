> **2026-09-23 预设状态使用范围已收紧（源码已验证，尚未本地安装）**：世界书完整保留15项预设并补齐11增益/4减益分类；新生成仅允许无剧情身份、机制简单的普通杂兵或偏中立卡牌使用，剧情相关敌人、复杂敌人和玩家整套卡组必须按剧情设定具体创作，不得套预设或只换名。七份世界书、生成/修复 Prompt 和 Schema 描述同源；不增加质量硬门槛，不改既有合法内容或存档。双类型、专项契约及隔离候选 `preset-scope-20260923-r1` 通过，1035源码冻结、1031保护文件未变。候选包含上轮尚未安装的MVU诊断修订；8012测试酒馆未启动，**两轮修订均尚未进入本地角色卡**。目标显示名 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**；未刷新/调用模型/推进聊天，无真实模型或实玩验收。详见 [预设状态创作范围](tasks/2026-09-23-preset-status-authoring-scope.md)。

> **2026-09-23 连续生成失败／预设 Prompt 核查（源码已验证，尚未本地安装）**：两份附件只证明 MVU 未得到更新块，第二份回退正文与原剧情楼层相同，均无提供方原始响应；上游失败根因仍待取证。已确认当前 1.0.3 世界书包含灵巧、遗忘及 15 项预设状态，但七个条目的旧“没有内置状态”说法冲突；本次统一白名单及请求/修复/Schema 提示，补助手完成输出与楼层解析的区分。双类型、专项回归及隔离候选 `mvu-prompt-audit-20260923-r1` 通过，原 dist/已安装扩展 1031 文件未变；旧 tower-request 文案断言失败已在 HEAD 复现。测试酒馆 8012 未启动，**本候选没有安装，不能把下方当前入口视为已含本次修订**。目标卡显示名 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**。未刷新/生成/推进聊天，未真实模型或实玩验收。详见 [失败诊断与 Prompt 核查](tasks/2026-09-23-generation-failure-prompt-audit.md)。
