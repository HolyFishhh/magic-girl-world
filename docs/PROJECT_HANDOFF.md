...
1. **正式发布版与当前开发态不是同一件事。** 当前正式角色卡仍是 `0.6.6`，正式设计辅助器仍是 `0.3.3`；本机 `codex/tower-mode` 工作树包含大量尚未提交的后续修改，不能把它写成已经发布的新版本。
2. **当前工作树不能只靠重新 clone 恢复。** 文档完成后的快照中，HEAD 为 `bd82b60`，另有 `167` 个已修改文件和 `34` 个未跟踪文件。迁移电脑前必须整体复制工作目录，或在用户明确授权后提交并推送；单纯 clone 只会得到 HEAD 之前的内容。
...
其中主项目最重要。除非用户明确要求，不要为了迁移擅自提交、推送、发版或清理工作树，也不要使用 `git reset --hard`、`git checkout --` 等会丢改动的命令。
...
- 主项目、角色卡、玩家日志和卡图仓库：等待用户明确要求后再提交、推送和发版。
- 扩展代码发生变化时采用独立持续发布，必须同步远端 `extension` 分支并验证远端 `manifest.json`；但纯文档任务没有扩展改动，不触发扩展发布。
