> **2026-09-22 流派与评分首批修复已本地安装（当前入口）**：源码 `5859abd`，候选 `scoring-20260922-r1`。修复评估缓存身份、欲望刷清敌进度、灵巧/零费漏识别、目标/因果误判、基础牌阈值断崖、无关状态加分与资源初始可用性；敌人周期按实际序列，被动未完整模拟时降低覆盖并说明。无伤害欲望对照从100%回到35%，96次正式试打、28组稳定性、15组定向检查及双类型通过；1027源码冻结、1031构建保护文件未变，卡内运行时/5项扩展HTTP哈希通过，348份其他角色/聊天/世界书未变。测试显示名 **魔法少女世界 1.0.3**，精确头像 **魔法少女世界 1.0.3.png**，用户自行刷新完整酒馆页。未调用真实模型或进行玩家实玩/新视觉验收。后续优先状态选择/转移、额外或替代费用；通用结算前拦截/永久移除限制尚未实现，已有缓冲/人工制品/无实体不能重复算缺失。详见 [修复、缺口与安装记录](tasks/2026-09-22-scoring-fixes-mechanism-gaps.md)。
