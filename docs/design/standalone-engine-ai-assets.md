...
执行顺序与自审修正见 [独立游戏执行方案](standalone-execution-plan.md)：默认以 Godot 标准版/GDScript 开始原型，先验证随包运行时分发与通信，再接清理后的真实核心战斗，之后接 AI。技术路线以这些门槛的实测结果放行。
...
首选技术切片分两步：E1 验证 Godot 导出窗口、中文/emoji、随包运行时通信及退出；E2 接清理后的 TS 核心，显示长中文卡面、目标选择与状态说明，播放必要出牌/结算反馈，验证选择和保存恢复。记录帧时间、内存、启动耗时和加载停顿，和当前相同场景比较。通过后再扩大客户端，不同时制作四套方案。
