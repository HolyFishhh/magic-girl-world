...
| 阶段 | 可交付结果 | 放行条件 |
| --- | --- | --- |
| P0 选型切片与基线清理 | 通用核心门面与宿主契约、Godot 交互切片、本地核心桥接、emoji 样板、去色情/欲望依赖清单与清理 | 独立消费者无需酒馆，类型/规则回归有效，输入法/长文本/动画与保存恢复可行；确定引擎 |
...
3. **建立独立消费入口**：补齐整局/爬塔/显示门面，定义动作、事件、快照与待处理选择的可序列化契约，接入参考战斗宿主；明确规则/内容/存档/桥接版本，用同一内容和随机源证明完整核心消费者的规则结果一致。保留 Toy/尖塔的适配边界，不实现其客户端。
