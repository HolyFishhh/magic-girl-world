...
日期：2026-09-22。最新需求：低价买断上架 Steam；以 AI 为主要特色；剧情模式必须接入 AI；爬塔提供一个简单但可完整游玩的离线预设，并支持 AI 自定义；脱离酒馆，采用类似 Codex 的工具调用智能体模式；彻底移除色情内容和欲望体系。美术沿用当前 emoji＋卡牌战斗，战斗可用游戏引擎重建，不受 HTML 限制；公开 AI 美术库仅为远期设想。卡牌核心需通用、便于更新，Bilibili Toy 与《杀戮尖塔 2》Mod 暂作架构考虑。本次仅规划，未开始机制清理或独立客户端实现。

本次补充约定：后续规划和执行由主代理直接处理，不再使用子代理。下文技术选择、内容数量与工作量均为规划建议；用户明确的产品需求为不可省略范围。

执行前自审已细化工程隔离、技术切片和放行条件，下一步按 [独立游戏执行方案](standalone-execution-plan.md) 的 E0—E6 推进；本页保留产品与总体架构。默认先验证 Godot 标准版/GDScript + 随包 TS/Node 运行时，不同时建设多个客户端。
...
通用化边界与二代 Mod 调研见 [通用卡牌核心与尖塔二代](portable-core-and-sts2-mod.md)。共享内容协议、规则、智能体与宿主适配分层；独立游戏消费完整内核，已有游戏的 Mod 可以仅消费可映射的内容子集。不能承诺在尖塔内部同时运行两套权威战斗规则，也不为潜在宿主提前重写或限制独立游戏核心。Toy 与 Mod 当前不进入实现排期。
...
- 角色、初始牌组、卡牌池，以及实际引用的状态、资源、召唤物、姿态、遗物定义；简单预设不必使用全部机制，未使用的通用机制通过独立契约样本验证。
...
此前 14—22 周估算基于 Web 界面和多套离线内容，已不适用于最新范围。技术切片也不以未经验证的 1—2 周估算锁定；先分别完成基线、导出通信与真实战斗，再根据实际结果估算首发。下表是总体阶段，详细工作包与放行条件以执行方案 E0—E6 为准。

| 阶段 | 可交付结果 | 放行条件 |
| --- | --- | --- |
| P0a 工程与分发（E0/E1） | 隔离源码基线、Godot 导出窗口、随包运行时、管道通信、中文/emoji 样例 | 不依赖开发工具 PATH；输入与错误反馈可用，正常/异常退出不遗留运行时 |
| P0b 核心与战斗（E2） | 去色情/欲望全链清理、通用门面、固定测试战斗、事务与存档 | 真实规则与双说明一致，选择/取消及稳定点恢复可行；通过后锁定该技术路线 |
...
P0 优先验证独立 Windows 导出与真实核心；Steam SDK 不作为起步依赖。P4/E6 接入所承诺的平台功能，成就、云存档和覆盖层分别验收，不把“窗口能打开”算成全部 Steam 功能已支持。JavaScript 等语言的 SDK 集成需要核对第三方绑定或实现本地桥接。[Steamworks API 接入说明](https://partner.steamgames.com/doc/sdk/api)
...
2. **验证分发与通信**：Godot 导出窗口启动随包运行时，完成握手、响应、中文/emoji 样例和异常退出；这一步不接模型，也不表示已有可玩战斗。
3. **清理主题与欲望协议**：在独立分支列出实际依赖并同步删除，更新中文说明、生成协议和测试夹具；保留通用资源和剧情链，不将新格式安装进当前酒馆。
4. **建立独立消费入口与存档**：补齐门面，定义动作、事件、快照和选择契约，接入参考战斗宿主；明确各版本、单一写入者和稳定保存边界，验证动作去重与恢复。保留 Toy/尖塔接口边界，不实现其客户端。
5. **接入固定战斗与现有美术**：核对所需 emoji/字体来源，沿用用途与卡牌美术；完成目标选择、回合、胜败、双说明和必要反馈，以相同输入/随机验证结果一致。这是开发测试场景，不是无 AI 剧情。
6. **建立独立智能体循环**：接模型适配器与状态/历史/规则查询、草稿、编译校验和提交工具；实现剧情到战斗再续写、上下文保存、预算停止与取消。用固定输入和受限真实请求分别验收。
7. **完成两种模式与离线爬塔**：补连续剧情、AI 爬塔和短而完整的本地爬塔；分别验证 AI 主链路和禁网预设、进程中断、取消、重复点击及奖励幂等，再扩展表现与 Steam 候选。
...
执行前已核对项目文档、源码边界和官方平台资料，并通过现有核心边界、参考运行时宿主及可移植类型检查，详细证据见执行方案。未构建桌面客户端、未测试 Steam SDK、未调用真实模型、未进行独立游戏实玩。文档提案没有运行时变化，因此不生成或安装新的酒馆角色卡；独立版采用隔离构建/数据目录，若另有回移酒馆维护线的运行时更新，仍按项目要求完成备份、哈希核验及本地测试卡同步。
