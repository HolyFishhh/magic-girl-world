...
日期：2026-09-22。最新需求：低价买断上架 Steam；以 AI 为主要特色；剧情模式必须接入 AI；爬塔提供一个简单但可完整游玩的离线预设，并支持 AI 自定义；脱离酒馆，采用独立多 agent 架构；彻底移除色情内容和欲望体系；采用精致 2D，初期美术以可商用 emoji 为主，逐步发展公开、AI 友好的美术素材库。本次仅规划，未开始机制清理或独立客户端实现。
...
建议首发 Windows、中文、单人、键鼠。AI 剧情和 AI 爬塔均为首发必需功能；基础离线爬塔用于教学、体验和规则回归。取消此前离线剧情、三套完整官方预设及其制作排期。画面方向已由用户确定为精致 2D：立绘、插画、卡牌动画与粒子；前期使用统一 emoji、排版、音效和动画形成完整体验。
...
首发暂不纳入联网对战、开发者代付模型服务、账号体系、创意工坊、手机端和完整模组编辑器。公开素材库先从本地目录和可公开清单演进，独立于游戏主线发布。Steam Deck、手柄与其他操作系统在分别验收前不作兼容承诺。
...
根据精致 2D 的最新方向，首选验证 **Godot 4 表现层 + 本地 TypeScript 游戏/AI 运行时**，以 **Phaser + Electron** 作为更侧重现有代码复用的备选。Godot 提供原生 2D、UI、动画和粒子，并采用 MIT 许可；这支持本项目选型判断，但不代表已做性能实测。[Godot 功能](https://docs.godotengine.org/en/stable/about/list_of_features.html)、[引擎许可](https://godotengine.org/license/)

Godot 不直接运行现有 TypeScript 模块，桥接、进程生命周期和安装包都是新增工作。首个原型把现有核心编译后放入随游戏启动的本地伴随进程，通过有版本的本机消息协议通信；玩家无需安装 Node 或启动酒馆。原型若显示该组合的分发/调试成本不合适，再选择 Phaser 方案。完整比较、agent 分工和素材规格见 [引擎、AI 与美术库设计](standalone-engine-ai-assets.md)。
...
flowchart TD
    ...
    APP --> ASSET[素材目录：本地 emoji 到公开 AI 素材库]
```
...
初版存档优先采用版本化 JSON 快照与原子替换，确有查询和数据量需要再评估数据库。Godot 界面重建时复用结构化说明和交互契约，先验证中文长文本、输入法、卡牌目标选择及动画中断。素材目录与 AI 编排独立于引擎，避免今后切换表现层时重做这两部分。
...
初期 emoji 选型为 **Twemoji 的图像素材**，固定发行版本/提交后以本地 SVG 源文件和构建生成的 PNG/图集使用，不依赖操作系统自带 emoji 字体。其图像采用 CC BY 4.0，可商用，需保留署名、来源、许可及修改说明；素材不得被重新标成全部自有或无条件使用。[Twemoji 授权说明](https://github.com/jdecked/twemoji#attribution-requirements)、[CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)

第一版投入统一卡面、布局、反馈、音效和手感。所有图片经语义素材 ID 与变体映射选择，例如 `element.fire`；后续可以换成自制 AI 图标或插画，而不改变卡牌规则和存档。缺图使用已打包的同类 emoji，并保留替代信息。公开库沿用同一编号体系，具体分层、检索接口和来源记录见配套设计。

## 6. AI 接入与存档

先支持一个明确的接口协议及少量实测服务，再扩展供应商适配。设置提供服务地址、模型、凭据、连通性测试和能力提示。叙事模型与机制模型可以共用配置，也可分别选择；结构化输出、流式输出和取消能力按适配器能力声明处理。

采用程序编排的专职 agent：导演负责场景与事实约束，叙事负责正文与选项，机制设计负责可执行卡牌/遭遇/状态变化；记忆整理、美术检索及按需审查作为辅助工作。agent 可共用一个模型 API，每次只获得相关上下文。简单对话可直接走叙事，复杂场景先形成共享方案，再进行互不依赖的生成工作；同一任务的依赖步骤不能盲目并行。

生成结果共同经过确定性编译校验、有限修复和唯一事务提交；只针对确定的错误槽位修复。AI 审查不能替代 Schema/执行校验，也不能按牌数或强度擅改合法设计。已经发布的叙事不因后台审查随意重写。普通战斗出牌仍由本地规则执行，不增加模型调用。

多 agent 的效果需要实测，不能预设必然优于单 agent。先用相同模型和总调用预算对比需求保留、连续性、首次可执行率、就绪延迟及用量，再确定哪些场景启用哪些角色；已有研究也显示收益取决于任务结构。[Google Research](https://research.google/blog/towards-a-science-of-scaling-agent-systems-when-and-why-agent-systems-work/)

为全部 agent 共用全局重试、并发和 token 预算，设置超时、取消和可恢复失败界面。使用运行 ID、请求 ID、基础状态版本拒绝迟到结果；模型失败不推进节点、不重复结算游戏资源。取消请求不等于服务商免收已产生费用，应用须如实记录已知用量。价格未知时只显示用量，避免虚构金额。首轮真实模型测试需事先限定样本及费用预算。
...
| P1 AI 主链路 | 自填 API、导演/叙事/机制角色、权威状态、有限修复 | 无酒馆完成 AI 剧情→事件/战斗→战后续写；多角色与基线对比留证 |
| P2 两种玩法与离线补充 | AI 连续剧情、AI 爬塔，一个基础离线爬塔 | 连续节点/跨幕、长剧情重开恢复通过；离线预设独立通关 |
| P3 体验与素材目录 | 统一 2D UI/动效/音效，语义素材目录、AI 选图、缓存和回退 | 长文本与操作不被动画阻塞；选图身份稳定，缓存缺失可恢复 |
| P4 Steam 候选 | 低价定位、分发构建、商店说明、平台功能与外部测试 | 干净机器启动、离线预设、AI 主功能和所宣称平台功能分别验收，完成送审 |
| P5 公开美术库 | 公开目录/API、许可元数据、分批 AI 美术、引擎导入器 | 素材可查找/预览/下载/归属追踪；独立发布，不阻塞游戏首发 |

首个核心成果改为“精致 2D 界面中，玩家输入想法，经 AI 生成可执行内容，选到对应 emoji，完成战斗并接续剧情，退出后仍可恢复”。另用固定内容演练禁网战斗和基础爬塔，不将它包装成离线剧情模式。
...
首发界面打磨包括：桌面全屏/窗口、字体缩放、键鼠导航、清晰的卡牌详情和目标选择、完整剧情记录、快捷查看牌库/弃牌堆、音量与减少动态效果设置。先建立统一的色彩、图标、字体、卡面和动效规则，再批量补美术。字体、音频和图像随包发布，生成的名称和长规则允许换行，重要选择不能被裁切。
...
5. **建立独立 AI 编排**：接模型适配器与受控工具，完成场景方案、叙事、机制、校验、提交及记忆更新；用固定输入和受限真实请求分别验收。
6. **接入 emoji 与表现层**：固定 Twemoji 版本、许可和素材清单，建立语义 ID；实现剧情/战斗/地图视图、输入法、目标选择、动效和设置。
...
商店明确“剧情模式需要模型 API；附带一个离线爬塔预设”，并披露外部调用费用。素材库的生成、存储和流量成本独立记账，初期游戏素材本地打包，避免售出一份游戏就承担无限在线成本。整理素材、字体、音频和代码的来源与许可，Twemoji 和未来自制素材分别记录。平台审核是独立里程碑，不能由本地测试替代。
...
候选版本必须分别提供基础离线爬塔完整通关、AI 剧情连续接续与重开、AI 爬塔连续节点及跨幕、引擎/伴随进程异常恢复、素材身份与缓存恢复、Steam 安装启动及所宣称功能的证据。真实模型失败按服务不可达、拒绝、格式错误、不可执行内容分别记录；不得以离线夹具通过代替真实模型体验。

主要风险及处理顺序：先验证引擎接入与状态脱离聊天楼层，同时衡量 AI 编排的质量、延迟及用量；再以完整剧情/爬塔验证接续，以素材目录验证风格和角色一致性，最后由外部玩家测试调整教学、节奏和表现。Steam BYOK 确认与 emoji 许可留存早期并行。新引擎表现和多 agent 质量都不能仅凭架构图宣称改善。
