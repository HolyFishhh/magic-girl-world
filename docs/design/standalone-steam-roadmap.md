...
主菜单提供“开始游戏”“继续游戏”“图鉴”“设置”。开始游戏先选“剧情冒险/爬塔挑战”，再选“官方预设/AI 自定义”，清楚区分玩法与内容来源。每个预设标注支持的玩法，不强迫同一内容包同时实现两种玩法。API 配置只在 AI 模式需要；预设模式第一次运行也不能依赖模型、在线字体、远程图片或开发服务器。
...
首发暂不纳入联网对战、开发者代付模型服务、账号体系、创意工坊、手机端和完整模组编辑器。保留接口扩展空间，但不让这些工作阻塞离线游戏、自由剧情与 Steam 发行。Steam Deck、手柄与其他操作系统在分别验收前不作兼容承诺。
...
初版存档优先采用版本化 JSON 快照与原子替换，减少额外原生依赖；确有查询和数据量需要再评估数据库。新界面先复用现有可信的显示组件和交互规则，统一挂载/卸载与状态订阅；首个原型不同时进行无必要的前端框架整体迁移。
...
内容生产顺序：先做一个基础构筑和短剧情切片，证明完整链路；扩成第一个正式预设后，再复用工具制作其余两套。为每套内容维护节点/事件清单、机制覆盖、文本和美术来源、人工通关记录。官方作者可以有意识地平衡内容，运行程序仍不得以质量评分改写玩家的合法 AI 内容。

建议三套内容各承担不同目的：第一套负责教学与人物剧情，第二套展示标准爬塔和路线构筑，第三套展示状态或召唤等复杂机制。暂不以虚构卡牌数量或承诺游玩小时数作为卖点；在玩家测试后确定并公开实际规模。
...
5. **实现本地内容提供器**：制作一套完整的短冒险，包含人物和剧情、战斗、选择、奖励及结局；所有资源随包，内容编译验证引用闭包。
6. **装入桌面客户端**：实现主菜单、剧情/战斗/地图视图和设置，接通完整动作链，输出可独立启动的 Windows 包。
7. **执行切片验收**：在隔离数据目录和禁网条件下，从干净安装到完整结局并重开存档；验证取消选牌、战斗中断、重复点击和奖励幂等；保存结果并修正后续估算。

P1 完成的判断是“没有酒馆、没有开发服务器、没有模型 API，玩家能玩完一段包含剧情与战斗的游戏并继续存档”。之后才扩为完整官方内容和真实 AI 自由冒险。
...
候选版本必须分别提供五类证据：离线剧情完整通关、离线爬塔完整通关、AI 剧情连续接续与重开、AI 爬塔连续节点及跨幕、Steam 安装启动与所宣称功能。真实模型失败要按服务不可达、拒绝、格式错误、不可执行内容分别记录；不得以离线夹具通过代替真实模型体验。

主要风险及处理顺序：先验证共享状态能否脱离聊天楼层，再验证完整离线内容覆盖；之后衡量 AI 的延迟、失败与用量，最后用外部玩家测试调整教学、节奏和表现。Steam BYOK 确认及素材授权核对应早期并行。任一阶段发现范围增大，以可玩切片证据重估后续排期，不压缩存档和事务验证来追求表面完工。
