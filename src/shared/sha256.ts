// SHA-256 constants from FIPS 180-4. The fallback supports HTTP LAN deployments,
// where browsers omit SubtleCrypto; persisted digests must remain identical.
...
export async function sha256(text: string): Promise<string> {
  ...
  data.setUint32(padded.length - 8, Math.floor(bytes.length / 0x20000000));
  data.setUint32(padded.length - 4, (bytes.length * 8) >>> 0);
