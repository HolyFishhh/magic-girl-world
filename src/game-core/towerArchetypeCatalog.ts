...
export const TOWER_ARCHETYPE_PRESETS: readonly TowerArchetypePreset[] = [
  ...
  foundation("status-copy", "status", "状态复制", "从符合筛选的状态中选择，复制给目标并保留状态身份与期限。", ["status_action"], ["status-stack"]),
  foundation("status-transfer", "status", "状态转移", "从符合筛选的状态中选择，转移给目标并保留状态身份与期限。", ["status_action"], ["status-stack"]),
  ...
  foundation("additional-payment", "cost", "额外支付", "为卡牌声明额外费用，出牌前完成原费用及额外费用的校验与支付。", [], ["cost-shift"]),
  foundation("alternative-payment", "cost", "替代支付", "为卡牌声明替代支付路径，出牌前选择一条可支付路径完成校验与支付。", [], ["cost-shift"]),
  ...
  foundation("damage-interception", "timing", "伤害前拦截", "让状态在 before_damage 时拦截该次伤害，可取消、替换或改写结算。", [], ["rule-control"]),
  foundation("card-play-interception", "timing", "出牌前拦截", "让状态在 before_card_play 时拦截该次出牌，可取消卡牌效果或替换为有限效果，已支付费用不退。", [], ["rule-control"]),
