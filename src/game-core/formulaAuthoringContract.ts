...
export function formulaAuthoringContractClauses(): string[] {
  return [
    ...
    '全局数值：spent_energy、x_value、turn_number、cards_played_this_turn、attacks_played_this_turn、skills_played_this_turn；spent_resource.资源ID、x_resource.资源ID。支付变量的适用位置见“资源与支付”。',
    '实体数值：以下每个路径都须加 self. 或 opponent. 前缀：hp、max_hp、lust、max_lust、energy、max_energy、block；hand_size、draw_pile_size、discard_pile_size、exhaust_pile_size；summon_count、ally_count；buff_count、debuff_count、neutral_count、status_count；status.状态ID.stacks；resource.资源ID.current/max。self 与 opponent 是并列根，不存在 self.opponent 或 opponent.self。',
