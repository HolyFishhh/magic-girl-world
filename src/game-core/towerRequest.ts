...
export function formatCompactEffectRepairContract(error: unknown): string {
  ...
  if (/INVALID_RELIC_RARITY|unsupported relic rarity|遗物 rarity 无效|遗物稀有度不受支持/i.test(detail)) {
    lines.push(
      '报错对象是遗物而不是卡牌：rarity 使用 Common/Uncommon/Rare/Epic/Legendary；Boss/ENS 只兼容旧内容。不支持 Corrupt 遗物。程序若已指定本槽位稀有度，按该稀有度设计，保留原机制语义，不得擅自降级。',
    );
  }
...
function createTowerInitialFiniteSupportStatusJsonSchema(id?: string): Record<string, unknown> {
  return {
    ...
    properties: {
      ...
      stun: { type: 'boolean' }, character_emoji: CHARACTER_EMOJI_SCHEMA, protection: DAMAGE_PROTECTION_SCHEMA,
      triggers: {
        type: 'object',
        additionalProperties: false,
        properties: {
