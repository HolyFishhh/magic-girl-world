...
export interface LifecycleCard {
  unique?: boolean; type?: string; retain?: boolean; exhaust?: boolean; ethereal?: boolean; innate?: boolean;
...
export function validateCardLifecycle(value: unknown): string | null {
  ...
  const allowed: Record<string,string[]> = {on_play:['discard','exhaust','purge'],on_discard:['discard','exhaust','purge'],turn_end:['discard','retain','exhaust']};
...
export function describeCardTraits(card: LifecycleCard): CardTrait[] {
  ...
  if(rule.turn_end==='exhaust') result.push({id:'ethereal',name:'空灵',detail:'回合结束仍在手中时消耗；正常打出是否消耗由打出特性决定。',tone:'fire'});
  if(rule.on_discard!=='discard') result.push({id:'discard-'+rule.on_discard,name:rule.on_discard==='purge'?'弃置销毁':'弃置消耗',detail:rule.on_discard==='purge'?'从手牌被主动或效果弃置后永久移除；打出和回合清理不触发。':'从手牌被主动或效果弃置后进入本场消耗区；打出和回合清理不触发。',tone:rule.on_discard==='purge'?'break':'fire'});
  return result;
}
