...
const TRIGGER_LABELS: Record<string, string> = {
  ...
  deal_lust_increase: '使对方欲望增加',
  deal_lust_decrease: '使对方欲望降低',
  gain_buff: '获得增益',
  gain_debuff: '获得减益',
  lose_buff: '失去增益',
  lose_debuff: '失去减益',
  enemy_gain_buff: '对方获得增益',
  enemy_gain_debuff: '对方获得减益',
  enemy_lose_buff: '对方失去增益',
  enemy_lose_debuff: '对方失去减益',
  gain_block: '获得格挡',
  lose_block: '失去格挡',
};
...
const VARIABLE_LABELS: Array<[RegExp, string]> = [
  ...
  [/\bself\.status\.([A-Za-z0-9_]+)\.stacks\b/g, '自身$1层数'],
  [/\bopponent\.status\.([A-Za-z0-9_]+)\.stacks\b/g, '对方$1层数'],
  [/\bstacks\b/g, '当前层数'],
  ...
  [/\bopponent\.max_hp\b/g, '对方最大生命'],
  [/\bopponent\.max_lust\b/g, '对方最大欲望'],
  [/\bopponent\.max_energy\b/g, '对方最大能量'],
  [/\bopponent\.hp\b/g, '对方生命'],
  [/\bopponent\.lust\b/g, '对方欲望'],
  [/\bopponent\.energy\b/g, '对方能量'],
  [/\bopponent\.block\b/g, '对方格挡'],
];
...
function formula(value: unknown): string {
...
function selectionText(value: Record<string, unknown>, defaultPick: 'random' | 'choose', count: unknown): string {
...
function displayStatusName(id: unknown, options: CompactCardDescriptionOptions): string {
  ...
  return options.statusNames?.[value]?.trim() || value;
}
...
function describeSingleOperation(
  ...
  const amount = formula(value[operation]);
  ...
  switch (operation) {
    case 'damage':
      text =
        target === '自身'
          ...
          : `造成${Number(value.hits) > 1 ? `${value.hits}次` : ''}${amount}点伤害`;
      break;
    case 'heal':
      text = target === '自身' ? `恢复${amount}点生命` : `使对方恢复${amount}点生命`;
      break;
    case 'block':
      text = target === '自身' ? `获得${amount}点格挡` : `使对方获得${amount}点格挡`;
      break;
    case 'energy':
      text = target === '自身' ? `获得${amount}点能量` : `使对方获得${amount}点能量`;
      break;
    case 'lust':
      text = target === '自身' ? `增加${amount}点欲望` : `使对方增加${amount}点欲望`;
      break;
    case 'set_hp':
    ...
    case 'narrate':
      ...
      break;
    case 'apply_status':
      text = `向${target}施加${formula(value.stacks ?? 1)}层${displayStatusName(value.apply_status, options)}`;
      break;
    case 'remove_status': {
    ...
    case 'seek':
      ...
      break;
    case 'discard':
      text = `弃掉${selectionText(value, 'random', value.discard)}`;
      break;
    case 'exhaust':
      text = `消耗${selectionText(value, 'random', value.exhaust)}`;
      break;
    case 'recover': {
    ...
    case 'reduce_cost':
      text = `使${selectionText(value, 'choose', value.count ?? 1)}费用降低${amount}`;
      break;
    case 'copy':
      text = `复制${selectionText(value, 'choose', value.copy)}到手牌`;
      break;
    case 'double':
      text = `使${selectionText(value, 'choose', value.double)}下次主效果执行两次`;
      break;
    case 'add_card': {
      const id = String(value.add_card);
      const cardName = templates.get(id) || id;
      const destination = value.to === 'deck' ? '抽牌堆' : '手牌';
      text = `将${formula(value.count ?? 1)}张${cardName}加入${destination}`;
      break;
    }
    case 'modify': {
      ...
      text = `${target}的${stats[String(value.modify)] || String(value.modify)}${verbs[operator]}${formula(value[operator])}`;
      break;
    }
  }
...
function describeEntry(
  ...
  if (typeof value.when === 'string' && value.when.trim()) text = `若${formula(value.when)}，${text}`;
...
function describeCompactMain(value: Record<string, unknown>, options: CompactCardDescriptionOptions): string {
  return typeof value.trigger === 'string'
    ? describeTriggeredEffectList(value.effects, value.creates, value.trigger, options)
...
export function describeCompactCard(value: unknown, options: CompactCardDescriptionOptions = {}): string {
  ...
  if (discarded) parts.push(`被效果弃掉时，${discarded}`);
