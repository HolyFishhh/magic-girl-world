...
'lifecycle.removable:false 表示不可永久移除：商店/营火/事件/删卡次数都不能删除；遗忘与打出销毁仍移出本场但战后保留原卡，普通消耗/遗弃不受限。lifecycle.transformable:false 独立禁止战斗和战外变形，仍允许升级/数值修改。两项默认 true；unique 不等于不可移除。',
'生命周期写在卡牌/模板根部，不是 effects 操作：lifecycle.on_play 选 discard/exhaust/purge，on_discard 选 discard/exhaust/remove/purge，turn_end 选 discard/retain/exhaust；仅写覆盖项。on_discard:"remove" 是“遗弃”，任意弃牌（包括回合清理）后移出本场且不可回收；on_discard:"purge" 是“遗忘”，任意弃牌后永久移除精确持有卡实例，临时副本不影响原卡；on_discard:"exhaust" 只是可回收的弃置消耗。正常打出不算弃牌。sly:true 是“灵巧”：回合清理前被主动或效果弃置时免费完整打出，回合末自动弃牌不触发。只需弃牌时发动单独效果，用根 discard_effects；需要弃牌时免费执行整张牌的正常 effects 才用 sly:true。同卡同时写两者会先结算 discard_effects 再完整打出；除非明确要两种独立效果，不要把同一收益重复配置。旧 exhaust/retain/ethereal/innate 仍可用，显式 lifecycle 按时机覆盖；Curse 默认保留，Power 默认打出消耗。临时牌由 add_card/复制生成，不给持有卡伪造 temporary 字段。',
