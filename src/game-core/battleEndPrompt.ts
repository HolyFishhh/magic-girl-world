...
export function formatBattleEndPrompt(input: BattleEndPromptInput): BattleEndPrompt {
  ...
  const enemies = input.enemies?.length ? [...input.enemies] : input.enemy ? [input.enemy] : [];
  ...
  if (enemies.length > 0) {
    summary.push('敌方：\n');
    for (const enemy of enemies) summary.push(
      `- ${enemy.name}：生命值${enemy.hp}/${enemy.maxHp}，欲望值${enemy.lust}/${enemy.maxLust}，能量${enemy.energy ?? 0}${enemy.maxEnergy === undefined ? '' : `/${enemy.maxEnergy}`}，格挡${enemy.block ?? 0}\n`,
      `  状态：${formatStatuses(enemy.statuses) || '无'}\n`,
    );
  } else {
    summary.push('敌方：无\n');
  }
