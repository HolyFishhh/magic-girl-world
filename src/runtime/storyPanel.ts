...
export function renderStoryPanel(view: 'common' | 'fish'): void {
  ...
  const displayedStory = current || (openingIsActive
    ? (initialScene || (isLaterAct ? '正在生成本幕开场剧情…' : ''))
    : latestVisitedStory || initialScene);
  const nodeId = currentNodeId;
  const renderKey = JSON.stringify([view, terminal, nodeId, battleTitle, displayedStory, initialScene, openingIsActive,
  ...
  for (const id of run?.visitedNodeIds || []) {
    if (id === currentNodeId || (!current && !openingIsActive && id === latestVisitedId)) continue;
