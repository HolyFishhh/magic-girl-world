...
2. 在角色管理中明确选择显示名为 `魔法少女世界 0.6.5` 的新卡，不要继续使用旧版本角色。
...
剧情模式只需角色卡。爬塔模式需要 `0.3.2` 或更高版本的“魔法少女世界设计辅助器”：开始页与设置悬浮窗会自动检测，缺失时可点击“快捷安装”，由 SillyTavern 官方安装器从本仓库 `extension` 分支安装；安装完成后刷新酒馆即可。早期手动复制的组件会自动迁移成可持续更新的仓库版本，不会保留两份。
