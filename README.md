...
2. 在角色管理中明确选择显示名为 `魔法少女世界 0.5.93` 的新卡，不要继续使用旧同名角色。
...
MVU 默认配置为：额外模型解析、自动请求，并只向额外模型发送 `[mvu_update]` 世界书。兼容假流式和“模型来源=与插头相同”由卡内加载器在首次安装时写入全局 MVU 设置，因为这两个字段不是 `[config_override]` 支持的角色级覆盖项。角色卡不保存 API Key；后续可在 MVU 面板切换独立模型。
...
`0.5.93` 的静态测量为：主模型全部剧情条目约 `2828` token；额外模型常驻约 `2860`；首轮完整内容约 `7702`；敌人注册约 `7558`。选择性生成协议只在对应控制标记出现时加入。
