...
新电脑或新开发环境请先阅读 [`docs/PROJECT_HANDOFF.md`](docs/PROJECT_HANDOFF.md)。该文档集中说明当前开发分支与正式发布版的差异、完整架构、剧情/MVU/战斗/爬塔链路、外部测试目录、真实模型证据、未提交工作树和当前卡点。历史重构日志只用于追溯，不应代替这份接手入口。

## 导入与启用

1. 在 SillyTavern 导入根目录 `魔法少女世界.png`。
2. 在角色管理中明确选择显示名为 `魔法少女世界 1.0.4` 的新卡，不要继续使用旧版本角色。
...
开始页只在首条 AI 消息渲染一次，提供剧情模式和爬塔模式入口。角色名、形象、世界观、身份、开场时机和卡牌体系都可留空；世界观与卡牌预设只提供方向，具体元素、状态和公式由剧情与 MVU 模型现场生成。变量模型直接使用 MVU 内置破限，无需额外安装或注册 preset；配置和调用时机见 `docs/mvu-extra-model-preset.md`。

剧情模式只需角色卡。爬塔模式需要 `1.0.4` 或更高版本的“魔法少女世界设计辅助器”：开始页与设置悬浮窗会自动检测，缺失或版本过旧时可点击安装/更新，由 SillyTavern 官方安装器从本仓库 `extension` 分支安装；安装完成后刷新酒馆即可。旧手动复制安装不再自动删除或迁移；需要自行安装当前版本组件。
...
MVU 默认配置为：额外模型解析、自动请求、内置破限、最近 2 层、最多 2 次顺序请求，并只向额外模型发送 `[mvu_update]` 世界书。角色卡不再注册、读取或切换任何 SillyTavern preset。兼容假流式和“模型来源=与插头相同”由卡内加载器初始化；正常第二轮保留当前模型与接口原生的推理能力，不强制开关 thinking/reasoning，也不注入供应商私有推理、温度或采样参数。结构修复和程序算分后的补全只通过提示词要求快速直出。顶部独立 MVU 浮窗显示第二轮的请求、生成、应用和完成进度；接口确实返回 reasoning 时可以查看，提供商未返回的隐藏思考过程无法显示。完成后的本轮变量变化由对应消息楼层中的独立 HTML 完整展示。角色卡不保存 API Key；后续可在 MVU 面板切换独立模型。
...
- 战斗页在桌面和手机 iframe 内避免页面整体横向滚动；手机手牌统一使用小扑克牌横向排列，卡内规则可滚动并可打开完整详情，桌面手牌按可用宽度排列。
...
- `src/game-core/`：无 Tavern、DOM、MVU 依赖的内容契约、公式、状态、牌区、回合、事务和战斗终态。
...
卡牌处理和战斗后端可从 `dist/portable/` 独立导入，不依赖酒馆助手、MVU 或 DOM。外部网站或 Mod 只需实现自己的状态、选择、事件和展示端口。
...
```bash
npm run tavern:readiness-chat -- valid "魔法少女世界.png"
npm run tavern:battle-repair-chat -- status "魔法少女世界.png" ordinary
```
...
剧情模式不依赖设计辅助扩展；爬塔模式依赖该组件完成单页地图、后台预生成、强度校准与存档恢复。

## 维护原则

- 后续更新按任务或独立阶段及时用 Git 提交保存，保证源码、测试、文档与发布产物可追溯；具体约定见 [项目开发约定](AGENTS.md) 和 [发布流程](docs/release-playbook.md)。
- 以最新版本内容为维护目标，下一版本清理不再需要的旧协议、迁移与兼容分支；不承诺历史内容持续可用，保留玩家原数据并说明升级影响。
