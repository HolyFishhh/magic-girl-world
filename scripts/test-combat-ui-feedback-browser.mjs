...
try {
  ...
  for(const width of [390,1000]){
    ...
    const removal=await evalJs(`(()=>{const c=document.querySelector('.shop-remove-confirm');return {enabled:!c.disabled,text:c.textContent,selected:document.querySelectorAll('.shop-remove-list .is-selected').length,extra:document.querySelectorAll('.mwg-card-preview').length,buttons:document.querySelectorAll('.shop-remove-list button').length}})()`);assert.equal(removal.enabled,true);assert.equal(removal.selected,1);assert.equal(removal.extra,0);assert.equal(removal.buttons,0);assert.match(removal.text,/75/);
    ...
    evidence.push({boundary,width,rules,stable,pov,escape,history,position,shop,removal,piles});
  }
