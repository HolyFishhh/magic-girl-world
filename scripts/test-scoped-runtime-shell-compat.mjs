...
const legacyFish='(?:<UpdateVariable>[\\s\\S]*?<\\/UpdateVariable>\\s*)?(?:<StatusPlaceHolderImpl\\s*\\/?>\\s*)?<BATTLE_START>\\s*(?:<StatusPlaceHolderImpl\\s*\\/?>)?';
const nativeStoryFish='(?:<(?:CHARACTER_INIT_PENDING|CONTENT_PENDING)>\\s*)*(?:<UpdateVariable>[\\s\\S]*?<\\/UpdateVariable>\\s*)?(?:<(?:CHARACTER_INIT_PENDING|CONTENT_PENDING)>\\s*)*(?:<StatusPlaceHolderImpl\\s*\\/?>\\s*)?<BATTLE_START>(?:\\s*<StatusPlaceHolderImpl\\s*\\/?>)?';
...
globalRule.findRegex=legacyFish;
shells[3].findRegex=nativeStoryFish;
...
assert.equal(apply(raw,[globalRule]),'剧情前文\n'+oldBody+'剧情后文','other characters still receive the unchanged old global rule');
...
assert.equal(compat.replaceString,'<BATTLE_START>\n');assert.equal(compat.substituteRegex,0);
...
assert.equal(apply(metacharBody,[metacharPlan.data.extensions.regex_scripts[0]]),'<BATTLE_START>\n','escape regex metacharacters literally');
