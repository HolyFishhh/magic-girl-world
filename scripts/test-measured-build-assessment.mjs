...
const trial=(seed,policy,extra={})=>({seed,policy,outcome:'horizon',turns:5,hpRemaining:70,damageDealt:40,enemyHpRemaining:360,cardsPlayed:10,deadTurns:1,lustDealt:0,horizons:[{turn:1,damage:8}],...extra});
const measurement=()=>({maxHp:70,evidence:BUILD_REFERENCE_CASES.map(reference=>({seeds:[1,2],trials:['tempo','survival','engine'].flatMap(policy=>[trial(1,policy,{enemyHpRemaining:reference.hp-40}),trial(2,policy,{enemyHpRemaining:reference.hp-40})])}))});
...
m=measurement();m.evidence[0].trials=[trial(1,'tempo',{outcome:'victory'}),trial(2,'tempo',{outcome:'defeat'}),trial(1,'survival',{damageDealt:200,enemyHpRemaining:200}),trial(2,'survival',{damageDealt:200,enemyHpRemaining:200})];r=assessMeasuredBuild(m);assert.equal(r.cases[0].best.policy,'survival');assert.deepEqual(r.cases[0].best.rows.map(row=>row.policy),['survival','survival']);
m=measurement();m.evidence.forEach(e=>e.trials.forEach(row=>{row.damageDealt=9999;row.enemyHpRemaining=0;row.outcome='defeat';}));assert.equal(assessMeasuredBuild(m).score,25);
...
const measured = assessMeasuredBuild({ maxHp: 100, evidence: [
  { seeds: [1], trials: [{ policy: 'tempo', seed: 1, outcome: 'horizon', damageDealt: 50, enemyHpRemaining: 350, hpRemaining: 60, cardsPlayed: 5, turns: 5, deadTurns: 0, lustDealt: 0, horizons: [{ turn: 1, damage: 4 }] }] },
  { seeds: [1], trials: [{ policy: 'tempo', seed: 1, outcome: 'victory', damageDealt: 100, enemyHpRemaining: 0, hpRemaining: 80, cardsPlayed: 5, turns: 5, deadTurns: 0, lustDealt: 0, horizons: [{ turn: 1, damage: 4 }] }] },
  { seeds: [1], trials: [{ policy: 'tempo', seed: 1, outcome: 'victory', damageDealt: 100, enemyHpRemaining: 0, hpRemaining: 80, cardsPlayed: 5, turns: 5, deadTurns: 0, lustDealt: 0, horizons: [{ turn: 1, damage: 4 }] }] },
], });
...
m=measurement();m.evidence.forEach((entry,index)=>entry.trials.forEach(row=>{
 const ratio=(index+1)/10;row.damageDealt=BUILD_REFERENCE_CASES[index].hp*ratio;row.enemyHpRemaining=BUILD_REFERENCE_CASES[index].hp*(1-ratio);row.hpRemaining=m.maxHp*ratio;
...
// Accumulated desire and damage do not replace observed enemy HP.
m=measurement();m.evidence.forEach((entry,index)=>entry.trials.forEach(row=>{row.enemyHpRemaining=BUILD_REFERENCE_CASES[index].hp;row.damageDealt=99999;row.lustDealt=99999;}));assert.equal(assessMeasuredBuild(m).score,35);
m.evidence[0].trials.forEach(row=>delete row.enemyHpRemaining);assert.equal(assessMeasuredBuild(m).score,null,'missing end-state evidence is unknown, never assumed cleared');
