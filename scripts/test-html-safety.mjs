...
const contracts = [
  ...
  ['src/fish/ui/battleShellPresenter.ts', /renderSupportDetails\(item,/, /escapeHtmlAttribute\(item\.id\)/],
  [
    'src/fish/ui/battleUI.ts',
    /renderCardFace\(cardData,/,
    /data-relic-description="\$\{escapeHtmlAttribute\(relic\.description/,
    /effectDisplay\.triggeredProgramToTags\(trigger, value\.effectProgram, displayContext, value\.eventQuery\)/,
    /effectDisplay\.programToTags\(value\.effectProgram, displayContext\)/,
    /data-ability-description="\$\{escapeHtmlAttribute\(description\)\}"/,
  ],
  ['src/fish/ui/pileViewer.ts', /renderCardFace\(card,/],
  ['src/shared/cardFace.ts', /escapeHtml\(cardData\.name\)/, /escapeHtml\(cardData\.description\)/, /escapeHtmlAttribute\(cardData\.id\)/],
  ['src/shared/supportPresentation.ts', /escapeHtml\(value\.name/, /escapeHtml\(value\.description\)/],
  ['src/fish/ui/cardInteractionPresenter.ts', /renderCardFace\(card,/, /escapeHtmlAttribute\(card\.id\)/],
  ['src/fish/ui/effectProgramDisplay.ts', /renderStatusReferences\(entry\.text,/, /escapeHtml\(entry\.icon\)/],
  ['src/shared/statusReference.ts', /result \+= escapeHtml\(text\.slice\(offset, next\)\)/, /data-status-rules="\$\{escapeHtml\(status\.rules/, /data-status-flavor="\$\{escapeHtml\(status\.flavor/],
