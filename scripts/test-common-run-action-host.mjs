...
const initialAcquisition = require(resolve('src/common/initialArtifactAcquisition.ts'));

const initialArtifact = { id: 'host_initial_relic', name: '入口遗物', rarity: 'Rare', on_acquire: { gold: 1 } };
function addPendingInitialArtifact(stat) {
  stat.battle = stat.battle || { core: { hp: 10, max_hp: 10, lust: 0, max_lust: 100, resources: [] }, cards: [], artifacts: [], items: [], statuses: [] };
  stat.battle.artifacts = [structuredClone(initialArtifact)];
  stat[initialAcquisition.INITIAL_ARTIFACT_ACQUISITION_KEY] = initialAcquisition.createInitialArtifactAcquisitionReceipt(
    stat.battle.artifacts, 'host-initial', stat.run?.stateRevision || 0,
  );
  return stat;
}
...
function createHarness(stat = {}, options = {}) {
  ...
  const ports = {
    ...
    initialPublicationReady: () => options.publicationReady !== false,
...
const pendingPublication = createHarness({}, { publicationReady: false });
await assert.rejects(pendingPublication.host.settleTowerOpeningChoice('a'), /保存确认/);
await assert.rejects(pendingPublication.host.activateTowerRunNode('a'), /保存确认/);
await assert.rejects(pendingPublication.host.retryTowerOpeningGeneration(), /保存确认/);
assert.equal(pendingPublication.updates(), 0, 'unconfirmed opening cannot be consumed or regenerated by stale UI callbacks');
...
const pendingEntryHarness = createHarness(addPendingInitialArtifact({
  game_mode: 'tower', game_mode_lock: { schemaVersion: 1, mode: 'tower' }, run: structuredClone(towerAwaiting),
}));
const pendingEntryBefore = structuredClone(pendingEntryHarness.stat());
await assert.rejects(pendingEntryHarness.host.enterTowerRunNode(towerChoice, 'blocked tower route'), /初始遗物/);
assert.deepEqual(pendingEntryHarness.stat(), pendingEntryBefore, 'a pending initial receipt blocks route entry without writing');
...
const blockedActivation = readyTowerBattleStat();
addPendingInitialArtifact(blockedActivation.stat);
const blockedActivationHarness = createHarness(blockedActivation.stat);
const blockedActivationBefore = structuredClone(blockedActivationHarness.stat());
await assert.rejects(blockedActivationHarness.host.activateTowerRunNode(blockedActivation.choice.id), /初始遗物/);
assert.deepEqual(blockedActivationHarness.stat(), blockedActivationBefore, 'a pending initial receipt blocks battle activation without consuming content');

const openingRun = core.createRunState({ seed: 9191 });
openingRun.opening = { phase: 'ready', requestId: 'opening', basedOnRevision: 0, attempts: 1, content: {
  title: '馈赠', narrative: '', choices: [{ id: 'take', label: '拿取', outcome: { gold: 1 } }],
} };
const blockedOpeningHarness = createHarness(addPendingInitialArtifact({
  run: openingRun, battle: { core: { hp: 10, max_hp: 10, lust: 0, max_lust: 100, resources: [] }, cards: [], artifacts: [], items: [], statuses: [] },
  reward: { card: [], artifact: [], item: [], limits: {} },
}));
const blockedOpeningBefore = structuredClone(blockedOpeningHarness.stat());
await assert.rejects(blockedOpeningHarness.host.settleTowerOpeningChoice('take'), /初始遗物/);
assert.deepEqual(blockedOpeningHarness.stat(), blockedOpeningBefore, 'opening settlement must not bypass an unhandled initial receipt');
...
const originalPool = structuredClone(rerollHarness.stat().reward);
rerollHarness.stat().run_reward_reroll = { original_reward: originalPool, categories: ['cards'], expected_counts: { cards: 1 } };
rerollHarness.stat().reward.card = [];
await rerollHarness.host.syncPendingRunState();
assert.deepEqual(rerollHarness.stat().reward, originalPool, 'interrupted old reroll restores the original candidates');
assert.equal(rerollHarness.stat().run.gold, 20);
assert.equal(rerollHarness.stat().run_reward_reroll, undefined);
assert.equal(rerollHarness.host.requestRewardReroll, undefined, 'no AI request entry remains');
await rerollHarness.host.syncPendingRunState();
assert.deepEqual(rerollHarness.stat().reward, originalPool, 'retirement is idempotent');
...
const eventRewardHarness = createHarness({
  ...
  reward: {
    card: [],
    artifact: [],
    item: [{ id: 'tonic', name: '星露', count: 1, effects: [{ heal: 3 }] }],
