...
const port=18177, out=resolve('tmp/pile-reward-v473-evidence'); fs.mkdirSync(out,{recursive:true});
const edge=spawn('C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe',['--headless=new','--disable-gpu','--no-first-run',`--remote-debugging-port=${port}`,`--user-data-dir=${resolve('tmp/pile-reward-browser-v473')}`,'about:blank'],{windowsHide:true,stdio:'ignore'});
let ws;
try {
  ...
  for(const width of [390,1000]){
    ...
    const batches=[];
    for(const direction of ['discard','draw']){
      await evalJs(`window.batchPromise=allCardPileFlow('${direction}');void 0`);await pause(115);await shot(direction+'-all-'+width);
      const batch=await evalJs('batchPromise');assert.deepEqual(batch.ids,['flow-0','flow-1','flow-2','flow-3','flow-4']);assert.equal(batch.groups,1);assert.equal(batch.particles,40);
      assert.equal(batch.faces,direction==='discard'?5:0);assert.equal(batch.hidden,direction==='draw'?5:0);assert.equal(batch.left,0);assert.equal(batch.visible,direction==='draw'?5:0);batches.push({direction,...batch});
    }
    const cancelled=await evalJs("allCardPileFlow('draw',true)");assert.equal(cancelled.left,0);assert.equal(cancelled.visible,5);
    await call('Emulation.setEmulatedMedia',{features:[{name:'prefers-reduced-motion',value:'reduce'}]});
    const reduced=await evalJs("allCardPileFlow('draw')");assert.equal(reduced.groups,0);assert.equal(reduced.hidden,0);assert.equal(reduced.visible,5);
    await call('Emulation.setEmulatedMedia',{features:[]});
    const timing=await evalJs('delayedDrawFixture()');assert.deepEqual(timing,{first:'hidden',rerender:'hidden',during:'hidden',after:'visible'});
    const death=await evalJs('deathLayoutFixture()');assert.equal(death.after.multi,true);for(const key of ['hand','party','x','y'])assert.ok(Math.abs(death.before[key]-death.after[key])<1,JSON.stringify(death));
    const loading=await evalJs('loadingFixture()');assert.deepEqual(loading,{visible:true,busy:'true',inert:true});await shot('loading-'+width);
    await evalJs('finishLoading()');assert.equal(await evalJs("document.getElementById('battle-loading').hidden"),true);assert.equal(await evalJs("document.querySelector('.battle-main-grid').inert"),false);
    ...
    evidence.push({width,layout,particles,batches,cancelled,reduced,timing,death,loading,cardText,popup});
  }
