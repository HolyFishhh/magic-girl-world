...
const port=18176, out=resolve('tmp/pile-reward-v472-evidence'); fs.mkdirSync(out,{recursive:true});
const edge=spawn('C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe',['--headless=new','--disable-gpu','--no-first-run',`--remote-debugging-port=${port}`,`--user-data-dir=${resolve('tmp/pile-reward-browser-v472')}`,'about:blank'],{windowsHide:true,stdio:'ignore'});
let ws;
try {
  ...
  for(const width of [390,1000]){
    ...
    evidence.push({width,layout,particles,batches,cancelled,reduced,cardText,popup});
  }
